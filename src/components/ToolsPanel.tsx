          ...
          {result && (
            <div className="bg-gradient-to-br from-gray-50 to-orange-50 dark:from-gray-700 dark:to-gray-700 p-4 rounded-xl border border-orange-200 dark:border-orange-800 space-y-2">
              <p className="flex justify-between text-sm text-gray-600 dark:text-gray-300">
                ...
                <span className="font-medium text-gray-700 dark:text-gray-200">{formatDate(result.startDate)} - {formatDate(result.endDate)}</span>
              </p>
              <p className="flex justify-between text-sm text-gray-600 dark:text-gray-300">
                <span>🧮 Günlük faiz (%5):</span>
                <span className="font-bold text-orange-600 dark:text-orange-400">{formatCurrency(result.dailyInterest)} ₺</span>
              </p>
              <p className="flex justify-between text-sm text-gray-600 dark:text-gray-300">
                <span>📌 Toplam faiz ({result.days} gün):</span>
                <span className="font-bold text-orange-600 dark:text-orange-400">{formatCurrency(result.totalInterest)} ₺</span>
              </p>
              ...
              <p className="flex justify-between text-base">
                ...
                <span className="font-bold text-green-600 dark:text-green-400 text-lg">{formatCurrency(result.total)} ₺</span>
              </p>
            </div>
          )}
        </div>
...
export default function ToolsPanel({ onAddNote, onAddTag, calculationId }: ToolsPanelProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [showPressDailyInterestModal, setShowPressDailyInterestModal] = useState(false);

  // Artık her zaman aktif - calculationId her zaman var (gerçek veya draft)
  const isDisabled = false;

  return (
    <>
      {/* Sağ kenar trigger butonu - Sadece tıklama ile açılır */}
      <div
        className="fixed right-0 top-1/2 -translate-y-1/2 z-50 cursor-pointer"
        onClick={() => setIsOpen(!isOpen)}
      >
        {/* Tab şeklinde gösterge - Gradient ve animasyonlu */}
        {!isOpen && (
        ...
        <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl shadow-2xl rounded-l-2xl border border-r-0 border-gray-200/50 dark:border-gray-700/50 p-5 min-w-[220px]">
          ...
          <div className="flex items-center justify-between mb-5 pb-3 border-b border-gray-200/50 dark:border-gray-700/50">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-gray-800 dark:text-gray-100 tracking-wide">Hızlı Araçlar</span>
            </div>
            <button
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <button
              ...
              className={`group relative flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-200 overflow-hidden ${
                isDisabled 
                  ...
                  : "bg-gradient-to-r from-amber-500 via-orange-500 to-red-500 hover:shadow-lg hover:shadow-orange-500/25 hover:-translate-y-0.5 text-white"
              }`}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700"></div>
              <div className={`relative z-10 w-9 h-9 rounded-lg bg-white/20 flex items-center justify-center ${!isDisabled && "group-hover:scale-110 group-hover:-rotate-3"} transition-transform duration-200`}>
            ...
            <button
              ...
              className={`group relative flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-200 overflow-hidden ${
                isDisabled 
                  ? "bg-gray-100 dark:bg-gray-800 text-gray-400 cursor-not-allowed" 
                  : "bg-gradient-to-r from-violet-500 via-purple-500 to-fuchsia-500 hover:shadow-lg hover:shadow-purple-500/25 hover:-translate-y-0.5 text-white"
              }`}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700"></div>
              <div className={`relative z-10 w-9 h-9 rounded-lg bg-white/20 flex items-center justify-center ${!isDisabled && "group-hover:scale-110 group-hover:rotate-6"} transition-transform duration-200`}>
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
