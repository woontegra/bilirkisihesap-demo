...
type Props = {
  ...
  buttonIcon?: ReactNode; // Solda gösterilecek ikon (Yazdır/Kaydet gibi)
  ...
  return (
    <>
      {!hideButton && (
        <button
          type="button"
          className={[buttonClassName || "bg-purple-600 hover:bg-purple-700 text-white text-xs sm:text-sm font-medium px-2 sm:px-3 py-1.5 rounded-md transition whitespace-nowrap", "flex items-center justify-center gap-1.5 sm:gap-2"].filter(Boolean).join(" ")}
          style={buttonStyle}
          onClick={handleButtonClick}
        >
          {buttonIcon ? <span className="shrink-0 flex items-center">{buttonIcon}</span> : null}
