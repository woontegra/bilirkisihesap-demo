...
// Constants - inline (Toplu İşçi Çıkarma)
...
const DOCUMENT_TITLE = "Mercan Danışmanlık | Toplu İş Sözleşmesi İhbar Tazminatı";
const PRINT_TITLE = "Toplu İş Sözleşmesi İhbar Tazminatı";
const PRINT_HEADING = "Toplu İş Sözleşmesi İhbar Tazminatı";
...
export default function IhbarTopluIndependent() {
  ...
  useEffect(() => {
    const calculateFromBackend = async () => {
      try {
        ...
        const result = await apiPost('/api/ihbar/toplu', requestData);
        if (result.success && result.data) {
          setWeeks(result.data.weeks || 2);
          setAmount(result.data.brut || 0);
          setGelirVergisi(result.data.gelirVergisi || 0);
          // gelirVergisiDilimleri string veya obje olarak gelebilir
          const dilimleriValue = result.data.gelirVergisiDilimleri;
          if (typeof dilimleriValue === 'string') {
            setGelirVergisiDilimleri(dilimleriValue);
          } else if (dilimleriValue && typeof dilimleriValue === 'object') {
            // Obje ise boş string kullan veya işlenmiş string döndür
