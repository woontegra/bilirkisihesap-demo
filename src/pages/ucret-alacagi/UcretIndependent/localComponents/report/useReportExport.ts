...
export function useReportExport(reportTitle: string, contentId: string = "report-content") {
  ...
  const handlePrint = () => {
    try {
      ...
      if (!targetEl) return;
      const html = "<!doctype html><html><head><meta charset=\"utf-8\"/><title>" + reportTitle + "</title><style>@page{size:A4 portrait;margin:12mm}*{box-sizing:border-box}body{font-family:Inter,Arial,sans-serif;color:#111827;padding:0;margin:0;font-size:10px;max-width:16cm}table{width:100%!important;max-width:16cm!important;border-collapse:collapse;margin-bottom:10px}thead{background:#f3f4f6}th,td{border:1px solid #999;padding:4px 6px;font-size:10px}th{text-align:left;font-weight:600}td{text-align:right}td:first-child{text-align:left}</style></head><body>" + targetEl.outerHTML + "</body></html>";
