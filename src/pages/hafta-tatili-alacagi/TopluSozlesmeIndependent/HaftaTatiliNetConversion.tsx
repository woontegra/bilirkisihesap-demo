...
export default function HaftaTatiliNetConversion({ haftaTatiliBrutTotal, tableData = [], dateRanges = [], onSummaryChange }: HaftaTatiliNetConversionProps) {
  ...
  useEffect(() => {
    const calculateFromBackend = async () => {
      try {
        console.log("[TopluHaftaTatiliNetConversion] Backend'e gönderiliyor:", { brutAmount: haftaTatiliBrut, year: selectedYear });
        ...
        if (!response.ok) {
          ...
          console.error("[TopluHaftaTatiliNetConversion] Backend hatası:", errorText);
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        
        const result = await response.json();
        console.log("[TopluHaftaTatiliNetConversion] Backend'den gelen sonuç:", result);
        ...
        console.error("[TopluHaftaTatiliNetConversion] Net hesaplama hatası:", error);
      }
    };
