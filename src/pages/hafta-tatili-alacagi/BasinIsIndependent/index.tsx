...
import BasinIsHaftaTatiliExpiryBox from "./BasinIsHaftaTatiliExpiryBox";
import BasinIsHaftaTatiliNetConversion from "./BasinIsHaftaTatiliNetConversion";
import BasinIsHaftaTatiliExcludeDays from "./BasinIsHaftaTatiliExcludeDays";
import BasinIsHaftaTatiliKatsayiModal from "./BasinIsHaftaTatiliKatsayiModal";
...
function generateBasinIsHaftaTatiliPeriods(workerStart: string, workerEnd: string): Array<{ start: string; end: string; wage: number }> {
...
function getBasinIsHaftaTatiliDaysForPeriod(
  periodStart: string,
  periodEnd: string,
...
export default function BasinIsIndependent() {
  ...
  const videoLink = getVideoLink("hafta-basin-is");
  ...
  // Basın İş Hafta Tatili Zamanaşımı state
  const [basinIsHaftaTatiliExpiryStart, setBasinIsHaftaTatiliExpiryStart] = useState<string | null>(null);

  // Basın İş Hafta Tatili Dışlanabilir günler state
  const [basinIsHaftaTatiliExcludedDays, setBasinIsHaftaTatiliExcludedDays] = useState<
    Array<{ id: string; type: "Yıllık İzin" | "Rapor" | "Diğer"; start: string; end: string; days: number }>
  >([]);

  // Sürekli gece çalışanı mı? (haftada 2 gün tatil)
  ...
  // Basın İş Hafta Tatili hesaplama tablosu
  const basinIsHaftaTatiliTableData = useMemo(() => {
    const allPeriodsWithStartDate: Array<BasinIsHaftaTatiliTableRow & { startDate: string }> = [];

    dateRanges.forEach((range) => {
      if (!range.start || !range.end) return;

      let effectiveStart: string;
      if (basinIsHaftaTatiliExpiryStart) {
        const rangeStartDate = new Date(range.start);
        const expiryStartDate = new Date(basinIsHaftaTatiliExpiryStart);
      ...
      const calculatedPeriods = generateBasinIsHaftaTatiliPeriods(effectiveStart, range.end);

      calculatedPeriods.forEach((period) => {
        const wage = period.wage;
        const coefficient = 1;
        const dailyWage = (wage * coefficient) / 30;
        
        const weekCount = calculateWeekCount(period.start, period.end, basinIsHaftaTatiliExcludedDays);
        
        const haftaTatiliDays = getBasinIsHaftaTatiliDaysForPeriod(
          period.start,
          period.end,
          selectedHolidayIds,
          basinIsHaftaTatiliExcludedDays
        );
        const dailyWage50Zamli = Number((dailyWage * 1.5).toFixed(2));
        // Sürekli gece çalışanları için haftada 2 gün tatil
        const haftaTatiliGunSayisi = isGeceCalisan ? 2 : 1;
        const haftaTatiliTotal = dailyWage50Zamli * weekCount * haftaTatiliGunSayisi;
  ...
  }, [dateRanges, selectedHolidayIds, basinIsHaftaTatiliExpiryStart, basinIsHaftaTatiliExcludedDays, isGeceCalisan]);

  // Basın İş Hafta Tatili Toplam Brüt Ücreti
  const basinIsHaftaTatiliTotalBrut = useMemo(() => {
    return basinIsHaftaTatiliTableData.reduce((sum, row) => sum + row.haftaTatiliTotal, 0);
  }, [basinIsHaftaTatiliTableData]);

  // Düzenlenebilir satırlar
  const [basinIsHaftaTatiliRows, setBasinIsHaftaTatiliRows] = useState<BasinIsHaftaTatiliTableRow[]>([]);
  ...
  const [basinIsHaftaTatiliNetSummary, setBasinIsHaftaTatiliNetSummary] = useState<{ brut: number; ssk: number; gelir: number; damga: number; net: number; hakkaniyet: number; settleAmount: string }>({ brut: 0, ssk: 0, gelir: 0, damga: 0, net: 0, hakkaniyet: 0, settleAmount: "" });

  // Taban veriler değiştiğinde tabloyu yenile
  useEffect(() => {
    setBasinIsHaftaTatiliRows(basinIsHaftaTatiliTableData);
    setHasCustomKatsayi(false);
  }, [basinIsHaftaTatiliTableData]);

  // Gece çalışanı seçimi değiştiğinde satırları yeniden hesapla
  useEffect(() => {
    if (basinIsHaftaTatiliRows.length > 0) {
      setBasinIsHaftaTatiliRows(prev => prev.map(r => recalcRow(r, isGeceCalisan)));
    }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const geceCalisanValue = formData.isGeceCalisan || formData.data?.form?.isGeceCalisan || formData.form?.isGeceCalisan;
        ...
        if (excludedDays && Array.isArray(excludedDays)) {
          setBasinIsHaftaTatiliExcludedDays(excludedDays);
        }
        if (zamanasimi?.start) {
          setBasinIsHaftaTatiliExpiryStart(zamanasimi.start);
        }
        if (periods && Array.isArray(periods)) {
          setBasinIsHaftaTatiliRows(periods);
        }
        if (typeof geceCalisanValue === 'boolean') {
          setIsGeceCalisan(geceCalisanValue);
        }
  ...
  const recalcRow = (row: BasinIsHaftaTatiliTableRow, geceCalisan: boolean = isGeceCalisan): BasinIsHaftaTatiliTableRow => {
    const dailyWage = (row.wage * row.coefficient) / 30;
    const dailyWage50Zamli = Number((dailyWage * 1.5).toFixed(2));
    // Sürekli gece çalışanları için haftada 2 gün tatil
    const haftaTatiliGunSayisi = geceCalisan ? 2 : 1;
    const haftaTatiliTotal = dailyWage50Zamli * row.weekCount * haftaTatiliGunSayisi;
    return { ...row, dailyWage, haftaTatiliTotal };
  };

  const createManualRow = useCallback((): BasinIsHaftaTatiliTableRow => {
    return {
      period: "",
      weekCount: 0,
      wage: 0,
  ...
  const duplicateRow = useCallback((i: number) => {
    setBasinIsHaftaTatiliRows((prev) => {
      const next = [...prev];
  ...
  const deleteRow = useCallback((i: number) => {
    if (basinIsHaftaTatiliRows.length <= 1) return;
    setBasinIsHaftaTatiliRows((prev) => prev.filter((_, idx) => idx !== i));
  }, [basinIsHaftaTatiliRows.length]);

  const handleWageChange = (index: number, value: string) => {
    const cleanValue = value.replace(/\./g, "").replace(",", ".");
    const wage = Number(cleanValue) || 0;
    setBasinIsHaftaTatiliRows(prev => prev.map((r, i) => i === index ? recalcRow({ ...r, wage }) : r));
  };

  const applyGlobalCoefficient = (k: number) => {
    const fixed = Number(k.toFixed(4));
    setBasinIsHaftaTatiliRows(prev => prev.map(r => recalcRow({ ...r, coefficient: fixed })));
    setHasCustomKatsayi(fixed !== 1);
  };

  const handleResetKatsayi = () => {
    setBasinIsHaftaTatiliRows(prev => prev.map(r => recalcRow({ ...r, coefficient: 1 })));
    setHasCustomKatsayi(false);
  };

  // Toplamı düzenlenebilir satırlardan hesapla
  const basinIsHaftaTatiliTotalBrutFromRows = useMemo(() => basinIsHaftaTatiliRows.reduce((s, r) => s + r.haftaTatiliTotal, 0), [basinIsHaftaTatiliRows]);
  ...
  const handleNewCalculation = () => {
    try {
      const hasUnsavedChanges = 
        ...
        basinIsHaftaTatiliRows.length > 0;
      ...
      setBasinIsHaftaTatiliExpiryStart(null);
      setBasinIsHaftaTatiliExcludedDays([]);
      setBasinIsHaftaTatiliRows([]);
      setCurrentRecordName(null);
      loadRanRef.current = false;
      
      if (id) {
        window.location.href = "/hafta-tatili-alacagi/basin-is";
      }
    } catch {}
  };
  ...
  const handleSave = () => {
    try {
      const katsayi = basinIsHaftaTatiliRows.length > 0 ? basinIsHaftaTatiliRows[0].coefficient : 1;
      ...
      const basinIsHaftaTatiliData = {
        periods: basinIsHaftaTatiliRows,
        totalBrut: basinIsHaftaTatiliTotalBrutFromRows,
        totalNet: basinIsHaftaTatiliNetSummary.net,
        netConversion: basinIsHaftaTatiliNetSummary,
        settlement: {
          hakkaniyet: basinIsHaftaTatiliNetSummary.hakkaniyet,
          settleAmount: basinIsHaftaTatiliNetSummary.settleAmount,
          sonuc: Math.max(0, basinIsHaftaTatiliNetSummary.brut - basinIsHaftaTatiliNetSummary.hakkaniyet),
        },
        workerPeriods: dateRanges,
        selectedHolidays: selectedHolidayIds,
        calculatedHaftaTatiliDays: totalDays,
        katsayi,
        zamanasimi: { active: !!basinIsHaftaTatiliExpiryStart, start: basinIsHaftaTatiliExpiryStart },
        excludedDays: basinIsHaftaTatiliExcludedDays,
        isGeceCalisan,
      ...
      kaydetAc({
        hesapTuru: "hafta_tatili_basin_is",
        veri: {
          data: {
            form: {
              workerPeriods: dateRanges,
              ...
              calculatedHaftaTatiliDays: totalDays,
              isGeceCalisan,
              settlement: basinIsHaftaTatiliData.settlement,
            },
            results: {
              totals: { brut: basinIsHaftaTatiliTotalBrutFromRows, net: basinIsHaftaTatiliNetSummary.net },
              brut: basinIsHaftaTatiliTotalBrutFromRows,
              net: basinIsHaftaTatiliNetSummary.net,
              netConversion: basinIsHaftaTatiliNetSummary,
            }
          },
          start_date: startDateStr,
          end_date: endDateStr,
          brut_total: basinIsHaftaTatiliTotalBrutFromRows,
          net_total: basinIsHaftaTatiliNetSummary.net,
          notes: "",
          ...basinIsHaftaTatiliData,
        },
        mevcutId: id,
        mevcutKayitAdi: currentRecordName,
        redirectPath: `/hafta-tatili-alacagi/basin-is/:id`,
      });
    } catch (e) {
  ...
  const basinHTReportConfig = useMemo((): ReportConfig => {
    const fmtLocal = (n: number) => n.toLocaleString("tr-TR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    
    const basinIsHaftaTatiliTotalAmount = basinIsHaftaTatiliNetSummary.brut || 0;
    const mahsuplasmaNum = Number(String(basinIsHaftaTatiliNetSummary.settleAmount || "0").replace(/\./g, '').replace(',', '.').replace('₺', '').trim()) || 0;
    const mahsuplamaSonucu = Math.max(0, basinIsHaftaTatiliTotalAmount - basinIsHaftaTatiliNetSummary.hakkaniyet - mahsuplasmaNum);

    return {
      title: "Basın İş Hafta Tatili Alacağı",
      sections: {
        info: true,
        periodTable: true,
        grossToNet: true,
      ...
      infoRows: [
        ...
        { label: "Sürekli Gece Çalışanı", value: isGeceCalisan ? "Evet" : "Hayır" },
        { label: "Zamanaşımı Başlangıç Tarihi", value: basinIsHaftaTatiliExpiryStart ? new Date(basinIsHaftaTatiliExpiryStart).toLocaleDateString("tr-TR") : "-", condition: !!basinIsHaftaTatiliExpiryStart },
      ],
      customSections: [
        {
          title: "Dışlanabilir Günler",
          content: basinIsHaftaTatiliExcludedDays.length > 0 ? (
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '16px' }}>
              ...
              <tbody>
                {basinIsHaftaTatiliExcludedDays.map((day, idx) => (
      ...
      periodData: {
        ...
        footer: [
          ...
          "",
          "",
          `${fmtLocal(basinIsHaftaTatiliTotalAmount)}₺`,
        ],
      ...
      grossToNetData: {
        ...
        rows: [
          { label: "Brüt Hafta Tatili Alacağı", value: `${fmtLocal(basinIsHaftaTatiliNetSummary.brut)}₺` },
          { label: "SGK İşçi Primi (%15)", value: `-${fmtLocal(basinIsHaftaTatiliNetSummary.ssk)}₺`, isDeduction: true },
          { label: "Gelir Vergisi", value: `-${fmtLocal(basinIsHaftaTatiliNetSummary.gelir)}₺`, isDeduction: true },
          { label: "Damga Vergisi (Binde 7,59)", value: `-${fmtLocal(basinIsHaftaTatiliNetSummary.damga)}₺`, isDeduction: true },
          { label: "Net Hafta Tatili Alacağı", value: `${fmtLocal(basinIsHaftaTatiliNetSummary.net)}₺`, isNet: true },
        ],
      },
      mahsuplasmaData: {
        title: "Mahsuplaşma",
        rows: [
          { label: "Net Hafta Tatili Alacağı", value: `${fmtLocal(basinIsHaftaTatiliTotalAmount)}₺` },
          { label: "1/3 Hakkaniyet İndirimi", value: `-${fmtLocal(basinIsHaftaTatiliNetSummary.hakkaniyet)}₺`, isDeduction: true },
  ...
  }, [basinIsHaftaTatiliRows, basinIsHaftaTatiliNetSummary, selectedHolidayIds, totalDays, basinIsHaftaTatiliExpiryStart, basinIsHaftaTatiliExcludedDays, isGeceCalisan]);

  const wordTableSections = useMemo(() => {
    const sections: Array<{ id: string; title: string; html: string }> = [];

    const infoRowsFiltered = (basinHTReportConfig.infoRows || []).filter((r) => r.condition !== false);
    ...
    if (basinIsHaftaTatiliExcludedDays.length > 0) {
      const excludedRows = basinIsHaftaTatiliExcludedDays.map((day) => [
        day.type,
    ...
    const pd = basinHTReportConfig.periodData;
    if (pd?.rows?.length) {
    ...
    const gnd = basinHTReportConfig.grossToNetData?.rows;
    if (gnd?.length) {
    ...
    const md = basinHTReportConfig.mahsuplasmaData;
    if (md?.rows) {
    ...
    return sections;
  }, [basinHTReportConfig, basinIsHaftaTatiliExcludedDays]);

  const handlePrint = useCallback(() => {
    const targetEl = document.getElementById("basin-is-ht-print-wrapper");
    if (!targetEl) {
      window.print();
      return;
    }
    const title = basinHTReportConfig.title;
  ...
  }, [basinHTReportConfig.title]);

  const handleBasinIsHaftaTatiliExpiryCancel = () => {
    ...
    >
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
        ...
        <div className="w-full space-y-6 max-w-full">
          ...
          {/* Dışlanabilir Günler */}
          <BasinIsHaftaTatiliExcludeDays
            basinIsHaftaTatiliExcludedDays={basinIsHaftaTatiliExcludedDays}
            onBasinIsHaftaTatiliExcludedDaysChange={setBasinIsHaftaTatiliExcludedDays}
          />
          </div>
        ...
        <div className="w-full mt-8 max-w-full">
          ...
          <Card className="w-full bg-white dark:bg-gray-800 rounded-3xl shadow-xl dark:shadow-gray-900/50 border border-gray-100 dark:border-gray-700 overflow-hidden">
            <CardHeader className="pb-3">
              ...
              <div className="flex flex-wrap justify-end gap-2 mt-2 w-full">
                <div className="flex items-center gap-2 flex-wrap">
                  <BasinIsHaftaTatiliExpiryBox
                    basinIsHaftaTatiliExpiryStart={basinIsHaftaTatiliExpiryStart}
                    onBasinIsHaftaTatiliExpiryStartChange={setBasinIsHaftaTatiliExpiryStart}
                    onBasinIsHaftaTatiliExpiryCancel={handleBasinIsHaftaTatiliExpiryCancel}
            ...
            <CardContent>
              {basinIsHaftaTatiliExpiryStart && basinIsHaftaTatiliRows.length > 0 && (
                <div className="mb-4 text-sm text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md p-2">
                  Zamanaşımı başlangıç tarihi: {format(new Date(basinIsHaftaTatiliExpiryStart), "dd.MM.yyyy")} — bu tarihten önceki dönemler cetvele dahil edilmemiştir.
                </div>
              )}
              {basinIsHaftaTatiliRows.length > 0 ? (
                <div className="w-full overflow-x-auto">
                  ...
                  >
                    ...
                    <tbody>
                      {basinIsHaftaTatiliRows.map((row, index) => (
                        <tr
                          key={index}
                              ...
                              <div className="flex gap-1 items-center">
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setBasinIsHaftaTatiliRows((prev) => prev.map((r, i) => {
                                      if (i !== index) return r;
                                      ...
                                      if (newStart && endISO) {
                                        newWeekCount = calculateWeekCount(newStart, endISO, basinIsHaftaTatiliExcludedDays);
                                      }
                                      const dailyWage50Zamli = Number((r.dailyWage * 1.5).toFixed(2));
                                      const haftaTatiliGunSayisi = isGeceCalisan ? 2 : 1;
                                      const haftaTatiliTotal = dailyWage50Zamli * newWeekCount * haftaTatiliGunSayisi;
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setBasinIsHaftaTatiliRows((prev) => prev.map((r, i) => {
                                      ...
                                      let newWeekCount = r.weekCount;
                                      if (startISO && newEnd) {
                                        newWeekCount = calculateWeekCount(startISO, newEnd, basinIsHaftaTatiliExcludedDays);
                                      }
                                      const dailyWage50Zamli = Number((r.dailyWage * 1.5).toFixed(2));
                                      const haftaTatiliGunSayisi = isGeceCalisan ? 2 : 1;
                                      const haftaTatiliTotal = dailyWage50Zamli * newWeekCount * haftaTatiliGunSayisi;
                          ...
                          <td className="bg-transparent w-16 p-0" style={{ borderTop: "1px solid #d2d2d2", borderBottom: "1px solid #d2d2d2", borderRight: "1px solid #d2d2d2" }}>
                            {hoveredRow === index && (
                              <span className="inline-flex items-center gap-1">
                                ...
                                <span
                                  className={`cursor-pointer ${basinIsHaftaTatiliRows.length <= 1 ? "text-gray-300 cursor-not-allowed" : "text-red-500 hover:text-red-600"}`}
                                  onClick={() => basinIsHaftaTatiliRows.length > 1 && deleteRow(index)}
                                  title={basinIsHaftaTatiliRows.length <= 1 ? "En az bir satır kalmalı" : "Satırı sil"}
                                >
                                  −
                                </span>
                              </span>
                    ...
                    <tfoot>
                      <tr className="bg-gray-50 dark:bg-gray-800 font-semibold">
                        ...
                        <td className="text-gray-900 dark:text-gray-100 text-right" style={{ padding: "8px", border: "1px solid #d2d2d2" }}>
                          {basinIsHaftaTatiliTotalBrutFromRows
        ...
        <div className="w-full mt-8 mb-24 md:mb-32 max-w-full">
          <div className="w-full">
            <BasinIsHaftaTatiliNetConversion 
              basinIsHaftaTatiliBrutTotal={basinIsHaftaTatiliTotalBrutFromRows} 
              tableData={basinIsHaftaTatiliRows}
              dateRanges={dateRanges}
              onSummaryChange={setBasinIsHaftaTatiliNetSummary} 
            />
          </div>
        </div>
          ...
          <div className="bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-800 dark:to-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            ...
            <div className="p-4 notes-content">
              <div className="space-y-2 text-sm text-slate-600 dark:text-slate-300">
                <p className="font-bold text-slate-800 dark:text-slate-200">Haftalık izin:</p>
                <div className="h-1" />
                <p className="font-medium">Madde 19 – (Değişik: 4/1/1961 - 212/1 md.)</p>
                <div className="h-1" />
                <p>Her altı günlük fiili çalışmayı mütaakıp gazeteciye bir günlük ücretli dinlenme izni verilmesi mecburidir. Gazetecinin vazifesi devamlı gece çalışmasını gerektirdiği hallerde hafta tatili iki gündür.</p>
                <div className="h-1" />
                <p>Birinci fıkra hükmü dışında gazeteci, çocuğu dünyaya geldiği zaman üç; eşi veya çocuğu, anası veya babası öldüğü zaman dört; çocuğu evlendiği, kardeşi, büyük anne veya büyük babası veya torunu öldüğü zamanlar iki gün olağanüstü ücretli izine hak kazanır. Bu izinler senelik izinden sayılmaz.</p>

                <div className="h-4" />

                <p className="font-bold text-slate-800 dark:text-slate-200">Çalışma Müddeti ve fazla mesai:</p>
                <div className="h-1" />
                <p className="font-medium">Ek Madde 1 – (Değişik: 12/2/1954 - 6253/2 md; Değişik: 4/1/1961 - 212/1 md.)</p>
                <div className="h-1" />
                <p>5953 sayılı Kanunun birinci maddesindeki gazeteci tabirinin şümulü içinde bulunan kimselerden müessese, matbaa, idarehane ve büro gibi yerlerde hizmetlerinin mahiyeti itibariyle müstemirren çalışanlar için günlük iş müddeti, gece ve gündüz devrelerinde sekiz saattir.</p>
                <div className="h-1" />
                <p>Yukarıki fıkra hükmünün dışında kalarak, gündüz veya gece devresindeki çalışma müddetinin daha fazla hadlere artırılması ve ulusal bayram, genel tatiller ve hafta tatilinde çalışılması bu kanuna göre (Fazla saatlerde çalışma) sayılır.</p>
                <div className="h-1" />
                ...
                <div className="h-1" />
                <p>Ancak, günlük normal çalışma müddetine ilaveten bu madde gereğince tatbik edilecek fazla çalışmaların saat 24 den sonraya tesadüf eden saatlerinde ücret bir misli fazlasiyle ödenir.</p>
                <div className="h-1" />
                <p>Fazla saatlerin hesabında, yarım saatten az olan müddetler yarım saat, fazlası ise bir saat sayılır.</p>
                <div className="h-1" />
                <p>Fazla saatlerde çalışma, ücretlerini parça başına veya yapılan iş miktarına göre alan gazetecilere yaptırıldığı takdirde dahi bu kimselerin fazla saatlere tekabül eden ücretleri bu maddedeki esaslara göre ödenir.</p>
                <div className="h-1" />
                <p>Fazla çalışmalara ait ücretin, mütaakıp ücret tediyesiyle birlikte ödenmesi mecburidir. (İptal ikinci cümle: Anayasa Mahkemesinin 19/9/2019 tarihli ve E.:2019/48; K.:2019/74 sayılı Kararı ile)</p>
                <div className="h-1" />
      ...
      <div id="basin-is-ht-print-wrapper" style={{ position: "absolute", left: "-9999px", top: 0, visibility: "hidden", width: "16cm", zIndex: -1 }} aria-hidden="true">
        <ReportContentFromConfig config={basinHTReportConfig} />
      </div>

      <BasinIsHaftaTatiliKatsayiModal open={showKatsayiModal} onClose={() => setShowKatsayiModal(false)} onApply={applyGlobalCoefficient} />
      ...
      <FooterActions 
        ...
        previewButton={{
          title: "Basın İş Hafta Tatili Alacağı Rapor",
          copyTargetId: "basin-is-ht-word-copy",
          ...
          renderContent: () => (
            <div style={{ background: "white", padding: 24 }}>
              <style>{`
                ...
                #basin-is-ht-word-copy table { border-collapse: collapse; width: 100%; margin-bottom: 12px; border: 1px solid #999; font-size: 9px; }
                #basin-is-ht-word-copy td { border: 1px solid #999; padding: 4px 6px; }
              `}</style>
              <div id="basin-is-ht-word-copy">
          ...
          onPdf: () => downloadPdfFromDOM("Basın İş Hafta Tatili Alacağı Rapor", "report-content"),
        }}
      />
    </Layout>
  );
