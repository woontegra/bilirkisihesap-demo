/**
 * LOCAL COPY - Copied from StandartIndependent
 * This file provides the report modal with export functionality
 */
...
export default function BaseReportModal({ open, onClose, config }: BaseReportModalProps) {
  ...
  return (
    ...
    >
      {/* Word sayfa genişliğine sığması için rapor max 16cm; mouse ile seçip yapıştırınca taşmaz */}
        ...
        <tbody>
          ...
          {config.customSections?.filter(section => section.condition !== false).map((section, idx) => (
            <React.Fragment key={idx}>
              {section.title && (
                <tr>
                  <td style={{ paddingBottom: '4px', borderBottom: '1px solid #e5e7eb', fontSize: '12px', fontWeight: 700, border: 'none' }}>
                    {section.title}
                  </td>
                </tr>
              )}
