    ...
    return (
      <div className="border rounded-lg p-4 bg-white">
        ...
        <div className="space-y-2">
          <div className="text-xs font-semibold text-gray-700">Grup 1</div>
          <div className="grid grid-cols-3 gap-2">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Gün Sayısı</label>
              <input
                type="number"
                min="0"
                ...
                onChange={(e) => {
                  const val = Math.min(7 - (pattern.days2 || 0), Math.max(0, parseInt(e.target.value) || 0));
                  const updatedPattern = { ...pattern, days1: val };
                  if (season === 'summer') {
                    updateWitness(witnessIndex, { summerPattern: updatedPattern });
                  } else {
                    updateWitness(witnessIndex, { winterPattern: updatedPattern });
                ...
                disabled={isReadOnly}
                className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
              />
            </div>
            <div>
        ...
        {/* Grup 2 */}
        <div className="space-y-2">
          <div className="text-xs font-semibold text-gray-700">Grup 2</div>
          <div className="grid grid-cols-3 gap-2">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Gün Sayısı</label>
              <input
                ...
                onChange={(e) => {
                  const val = Math.min(7 - (pattern.days1 || 0), Math.max(0, parseInt(e.target.value) || 0));
                  const updatedPattern = { ...pattern, days2: val };
                  if (season === 'summer') {
                    updateWitness(witnessIndex, { summerPattern: updatedPattern });
                  } else {
                    updateWitness(witnessIndex, { winterPattern: updatedPattern });
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  const updatedPattern = { ...pattern, startTime2: e.target.value };
                  if (season === 'summer') {
                    updateWitness(witnessIndex, { summerPattern: updatedPattern });
                  } else {
                    updateWitness(witnessIndex, { winterPattern: updatedPattern });
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  const updatedPattern = { ...pattern, endTime2: e.target.value };
                  if (season === 'summer') {
                    updateWitness(witnessIndex, { summerPattern: updatedPattern });
                  } else {
                    updateWitness(witnessIndex, { winterPattern: updatedPattern });
          ...
          <div className="space-y-2 pt-2 border-t border-gray-200">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) => {
                  const updatedPattern = { ...pattern, hasWeeklyHoliday: e.target.checked };
                  if (season === 'summer') {
                    updateWitness(witnessIndex, { summerPattern: updatedPattern });
                  } else {
                    updateWitness(witnessIndex, { winterPattern: updatedPattern });
              ...
              />
              <span className="text-sm font-medium text-gray-700">Hafta Tatili Var mı?</span>
            </label>
            ...
            {pattern.hasWeeklyHoliday && (
              <div>
                ...
                <select
                  value={pattern.weeklyHolidayRow || 1}
                  onChange={(e) => {
                    const updatedPattern = { ...pattern, weeklyHolidayRow: parseInt(e.target.value) };
                    if (season === 'summer') {
                      updateWitness(witnessIndex, { summerPattern: updatedPattern });
                    } else {
                      updateWitness(witnessIndex, { winterPattern: updatedPattern });
    ...
    <div className="space-y-4">
      ...
      {witnesses.length === 0 && (
        <div className="text-center py-8 text-gray-500 text-sm">
          Henüz tanık eklenmedi. Aşağıdaki "Tanık Ekle" butonuna tıklayarak tanık ekleyebilirsiniz.
        </div>
      )}
      ...
      {/* Tanık Ekle Butonu - En Alta - Tam Sayfa Genişliğinde */}
      {!isReadOnly && (
        <button
          type="button"
          onClick={addWitness}
          className="w-full px-6 py-3 bg-blue-500 text-white text-sm font-medium rounded-md hover:bg-blue-600 transition-colors shadow-md hover:shadow-lg mt-4"
        >
          + Tanık Ekle
        </button>
      )}
