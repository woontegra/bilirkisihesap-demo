...
function FazlaMesaiBilirkisi1PageContent({ titleOverride }: Props) {
  ...
  const derivedRows = useMemo(() => {
    ...
    if (!witnesses || witnesses.length === 0) {
      console.log('[HAFTALIK_KARMA CALC] Tanık yok');
      return [];
    }
    ...
    const allWitnesses = witnesses.map((w: any) => {
    ...
    console.log('[HAFTALIK_KARMA CALC] Split edilmiş tanıklar:', splitWitnesses.length);
    
    // generateDynamicIntervalsFromWitnesses ile birleşik aralıkları al
    let intervals = generateDynamicIntervalsFromWitnesses(davaci, splitWitnesses);
    
    console.log('[HAFTALIK_KARMA CALC] Intervals:', intervals.length);
    
    if (!intervals || intervals.length === 0) {
      console.log('[HAFTALIK_KARMA CALC] Intervals boş');
      return [];
    }
    
    // HER INTERVAL İÇİN ASGARI ÜCRET DÖNEMLERİNE BÖL VE SATIR OLUŞTUR
    ...
    console.log('[HAFTALIK_KARMA CALC] Generated rows:', generatedRows.length);
  ...
  // HAFTALIK_KARMA PAGE - Only sync haftalikKarmaFinalRows
  useEffect(() => {
    // Only sync if data actually changed (check weeks, fmHours, katsayi for changes)
    if (haftalikKarmaFinalRows.length === derivedRowsWithExclusions.length &&
        haftalikKarmaFinalRows.length > 0 &&
        haftalikKarmaFinalRows[0]?.id === derivedRowsWithExclusions[0]?.id &&
        haftalikKarmaFinalRows[0]?.weeks === derivedRowsWithExclusions[0]?.weeks &&
        haftalikKarmaFinalRows[0]?.fmHours === derivedRowsWithExclusions[0]?.fmHours &&
        haftalikKarmaFinalRows[0]?.katsayi === derivedRowsWithExclusions[0]?.katsayi) {
      console.log('⏭️ [SYNC HAFTALIK_KARMA] Skipping sync - data unchanged');
      return;
    }
    
    console.log('🔄 [SYNC HAFTALIK_KARMA] Syncing finalRows:', derivedRowsWithExclusions.length, 'rows');
    setHaftalikKarmaFinalRows([...derivedRowsWithExclusions]);
    rowsRef.current = derivedRowsWithExclusions;
  }, [derivedRowsWithExclusions, haftalikKarmaFinalRows]);
  ...
  const fmPeriods = useMemo(() => {
    ...
    if (!haftalikKarmaState.weeklyStartDateISO || !haftalikKarmaState.weeklyEndDateISO) {
      ...
      // Helper function: Hukuki ara dinlenme hesabı (4857 m.68 + Yargıtay)
      ...
      const generateWeeklyText = (dayGroups: any[], title: string) => {
        ...
        const weeklyOvertime = Math.max(0, totalNet - 45);
        
        // Format: "Toplam çalışma = 28,5 saat + 37,5 saat = 66 saat"
        const groupSums = groupTotals.map(g => `${fmt(g)} saat`).join(' + ');
        textLines.push(`Toplam çalışma = ${groupSums} = ${Math.round(totalNet)} saat`);
        textLines.push(`Net haftalık çalışma = ${totalNet.toFixed(2)} saat,`);
        textLines.push(`${totalNet.toFixed(2)} – 45 saat yasal haftalık çalışma = ${weeklyOvertime.toFixed(2)} saat haftalık fazla mesai`);
      ...
      // 2) Her TANIK için ayrı metin (DAVACI SAATLERİYLE KESİŞİM)
      if (haftalikKarmaState.witnesses && haftalikKarmaState.witnesses.length > 0) {
        // Davacı saatlerini al
        const davaciGroups = haftalikKarmaState.dayGroups || [];
        const davaciStartTime = davaciGroups[0]?.startTime || '07:00';
        const davaciEndTime = davaciGroups[0]?.endTime || '18:00';
        ...
        haftalikKarmaState.witnesses.forEach((witness, idx) => {
          ...
          witnessGroups = witnessGroups.map(group => {
            const [tGirH, tGirM] = (group.startTime || '07:00').split(':').map(Number);
            const [tCikH, tCikM] = (group.endTime || '18:00').split(':').map(Number);
            const [dGirH, dGirM] = davaciStartTime.split(':').map(Number);
            const [dCikH, dCikM] = davaciEndTime.split(':').map(Number);
            ...
            // KESİŞİM: MAX(tanık giriş, davacı giriş), MIN(tanık çıkış, davacı çıkış)
    ...
    return results;
  }, [haftalikKarmaState.weeklyStartDateISO, haftalikKarmaState.weeklyEndDateISO, haftalikKarmaState.dayGroups, haftalikKarmaState.witnesses]);
