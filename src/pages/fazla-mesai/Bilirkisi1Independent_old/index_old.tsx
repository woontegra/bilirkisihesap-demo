...
function FazlaMesaiBilirkisi1PageContent({ titleOverride }: Props) {
  ...
  // Detect scenario from route and set calculationScenario
  useEffect(() => {
    const path = location.pathname;
    let targetScenario: typeof calculationScenario = calculationScenario;
    
    if (path.includes('/standart')) {
      targetScenario = 'STANDART';
    } else if (path.includes('/haftalik-karma')) {
      targetScenario = 'HAFTALIK_KARMA';
    } else if (path.includes('/donemsel-karma')) {
      targetScenario = 'DONEMSEL_HAFTALIK_KARMA';
    } else if (path.includes('/donemsel')) {
      targetScenario = 'DONEMSEL';
    }
    
    // Only update if scenario actually changed
    if (targetScenario !== calculationScenario) {
      setCalculationScenario(targetScenario);
    }
  }, [location.pathname, calculationScenario, setCalculationScenario]);
  ...
  // KRİTİK: HAFTALIK_KARMA için SADECE fmPeriods (tanık kesişimi sonucu)
  const activeRows = useMemo(() => {
    if (calculationScenario === 'STANDART') {
      return standartFinalRows;
    }
    if (calculationScenario === 'HAFTALIK_KARMA') {
      // HAFTALIK_KARMA: SADECE fmPeriods (tanık kesişimi SONRASI)
      return haftalikKarmaFinalRows;
    }
    return [];
  }, [calculationScenario, standartFinalRows, haftalikKarmaFinalRows]);
  ...
  // STANDART SCENARIO - Kendi state'i ile çalışır
  const standartRows = useStandartScenario({
    standardState,
    weeklyDays,
    activeTab,
    katSayi,
    ...
    mode270,
    rows,
    witnessIntersectionFMRef
  });

  // HAFTALIK KARMA SCENARIO - RAW daily rows üretir
  const haftalikKarmaRawRows = useHaftalikKarmaScenario({
    haftalikKarmaState
  });

  // KRİTİK: RAW ROWS'ı davaci/taniklar formatına çevir
  // Sonra AYNI StandartScenario chain'inden geçir
  const haftalikKarmaVirtualState = useMemo(() => {
    if (!haftalikKarmaRawRows || haftalikKarmaRawRows.length === 0) {
      return { davaci: { dateIn: '', dateOut: '', in: '', out: '' }, taniklar: [] };
    }
    
    const davaciRows = haftalikKarmaRawRows.filter(r => r.type === 'davaci');
    const tanikRows = haftalikKarmaRawRows.filter(r => r.type === 'tanik');
    
    if (davaciRows.length === 0) {
      return { davaci: { dateIn: '', dateOut: '', in: '', out: '' }, taniklar: [] };
    }
    
    const firstDavaci = davaciRows[0];
    const lastDavaci = davaciRows[davaciRows.length - 1];
    
    const davaci = {
      dateIn: firstDavaci.dateISO,
      dateOut: lastDavaci.dateISO,
      in: firstDavaci.in,
      out: firstDavaci.out
    };
    
    const witnessMap = new Map<number, any>();
    tanikRows.forEach(row => {
      if (!row.witnessId) return;
      
      if (!witnessMap.has(row.witnessId)) {
        witnessMap.set(row.witnessId, {
          id: row.witnessId,
          dateIn: row.dateISO,
          dateOut: row.dateISO,
          in: row.in,
        ...
        const existing = witnessMap.get(row.witnessId);
        if (row.dateISO < existing.dateIn) existing.dateIn = row.dateISO;
        if (row.dateISO > existing.dateOut) existing.dateOut = row.dateISO;
      }
    });
    
    const taniklar = Array.from(witnessMap.values());
    
    return { davaci, taniklar };
  }, [haftalikKarmaRawRows]);

  // HAFTALIK_KARMA: AYNI StandartScenario hook'unu kullan
  const haftalikKarmaRows = useStandartScenario({
    standardState: haftalikKarmaVirtualState,
    weeklyDays,
    activeTab,
    katSayi,
    zamanasimiBaslangic,
  ...
  // DERIVED ROWS - Aktif senaryoya göre seç
  const derivedRows = useMemo(() => {
    switch (calculationScenario) {
      case "STANDART":
        return standartRows;
      case "HAFTALIK_KARMA":
        return haftalikKarmaRows;
      case "DONEMSEL":
        return [];
      case "DONEMSEL_HAFTALIK_KARMA":
        return [];
  ...
  }, [calculationScenario, standartRows, haftalikKarmaRows]);
  ...
  // ONLY for HAFTALIK_KARMA route - independent of STANDART
  useEffect(() => {
    // Route guard: Only run for /haftalik-karma route
  ...
  const fmPeriods = useMemo(() => {
    ...
    switch (calculationScenario) {
      ...
      case "DONEMSEL":
      case "DONEMSEL_HAFTALIK_KARMA":
        // Bu senaryolar için henüz metin hesaplaması yok
        return [];
      default:
        return [];
    }
...
= ${calculatedWeeklyFM.toFixed(2)} saat haftalık fazla mesai`;
  ...
  }, [finalRows, standardState.davaci, standardState.taniklar, weeklyDays, activeTab, calculationScenario, haftalikKarmaState.weeklyStartDateISO, haftalikKarmaState.weeklyEndDateISO, haftalikKarmaState.dayGroups]);
  ...
  */
  ...
  /** Footer sol tarafı: 270 Saat Düşüm, Zamanaşımı İtirazı, Kat Sayı Hesapla (dropdown yukarı açılır) */
  const footerLeftContent = (
    <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-sm">
      {/* 270 SAAT DÜŞÜM - DROPDOWN (yukarı açılır) */}
      <div className="relative">
        {show270Dropdown && (
          <div className="fixed inset-0 z-[45]" onClick={() => setShow270Dropdown(false)} />
        )}
        <button
          type="button"
        ...
        >
          ...
          <svg className={`w-4 h-4 transition-transform duration-200 ${show270Dropdown ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
          </svg>
        </button>
        {show270Dropdown && (
          <div className="absolute bottom-full left-0 mb-2 w-64 bg-white border border-gray-100 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in duration-200">
            <button
              type="button"
              onClick={() => {
                setMode270("none");
              ...
              className={`w-full text-left px-4 py-3 hover:bg-purple-50 transition-colors border-b border-gray-100 ${mode270 === "none" ? "bg-purple-50" : ""}`}
            >
              <div className="flex items-start gap-3">
                <div className={`mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center ${mode270 === "none" ? "border-purple-500 bg-purple-500" : "border-gray-300"}`}>
                  {mode270 === "none" && <div className="w-2 h-2 bg-white rounded-full" />}
                </div>
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 hover:bg-purple-50 transition-colors border-b border-gray-100 ${mode270 === "detailed" ? "bg-purple-50" : ""}`}
            >
              <div className="flex items-start gap-3">
                <div className={`mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center ${mode270 === "detailed" ? "border-purple-500 bg-purple-500" : "border-gray-300"}`}>
                  {mode270 === "detailed" && <div className="w-2 h-2 bg-white rounded-full" />}
                </div>
                <div className="flex-1">
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 hover:bg-purple-50 transition-colors ${mode270 === "simple" ? "bg-purple-50" : ""}`}
            >
              <div className="flex items-start gap-3">
                <div className={`mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center ${mode270 === "simple" ? "border-purple-500 bg-purple-500" : "border-gray-300"}`}>
                  {mode270 === "simple" && <div className="w-2 h-2 bg-white rounded-full" />}
                </div>
                <div className="flex-1">
      ...
      <button
        ...
        className={`inline-flex items-center gap-2.5 px-4 py-2 text-sm font-medium rounded-full border transition-all duration-200 ${
          zamanasimiBaslangic ? "bg-gradient-to-r from-blue-500 to-cyan-600 text-white border-transparent shadow-md hover:from-blue-600 hover:to-cyan-700" : "bg-white text-gray-700 border-gray-300 hover:border-blue-400 hover:bg-blue-50 hover:text-blue-600"
        }`}
      >
        {zamanasimiBaslangic && (
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <button
        ...
        className={`inline-flex items-center gap-2.5 px-4 py-2 text-sm font-medium rounded-full border transition-all duration-200 ${
          hasCustomKatsayi ? "bg-gradient-to-r from-green-500 to-emerald-600 text-white border-transparent shadow-md hover:from-green-600 hover:to-emerald-700" : "bg-white text-gray-700 border-gray-300 hover:border-green-400 hover:bg-green-50 hover:text-green-600"
        }`}
      >
        {hasCustomKatsayi && (
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {showZamanaModal && (
            ...
            {calculationScenario ? (
              ...
              <div className="p-4 space-y-4">
                ...
                {/* DONEMSEL: Çoklu dönem beyan yönetimi (SINGLE pattern only) */}
                {calculationScenario === "DONEMSEL" && (
                  <div>
                    <div className="text-xs text-gray-600 mb-3 italic">Dönemsel Giriş (Her dönem tek tip saat)</div>
                    <DavaciDeclarationManager
                      declaration={donemselState.davaciDeclaration}
                      onUpdate={(updatedDeclaration) => {
                        setDonemselState(prev => ({ ...prev, davaciDeclaration: updatedDeclaration }));
                      }}
                      isReadOnly={isReadOnly}
                      scenario="DONEMSEL"
                    />
            ...
            {calculationScenario ? (
              ...
              <div className="p-4 space-y-4">
                ...
                {/* DONEMSEL: Çoklu dönem tanık beyan yönetimi (SINGLE pattern only) */}
                {calculationScenario === "DONEMSEL" && (
                  <div>
                    <div className="text-xs text-gray-600 mb-3 italic">Dönemsel Giriş (Her dönem tek tip saat)</div>
                    <TanikDeclarationManager
                      declarations={donemselState.tanikDeclarations}
                      onUpdate={(updatedWitnesses) => {
                        setDonemselState(prev => ({ ...prev, tanikDeclarations: updatedWitnesses }));
                      }}
                      isReadOnly={isReadOnly}
                      scenario="DONEMSEL"
                    />
