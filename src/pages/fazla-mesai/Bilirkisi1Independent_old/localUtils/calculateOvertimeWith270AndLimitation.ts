...
// - Yıllık izin düşümü EN SON (gün/6 + ROUND)
...
export function calculateOvertimeWith270AndLimitation(params: {
  ...
  while (yilBaslangic <= istenCikisTarihi) {
    ...
    // FM başlangıç tarihi (270 sonrası)
    const fmBaslangic = new Date(yilBaslangic.getTime() + dusulecekHafta * HAFTA)
  ...
  // ========================================
  // ADIM 3: YILLIK İZİN DÜŞÜMÜ (EN SON, /6 + ROUND)
  // ========================================
  if (yillikIzinler && yillikIzinler.length > 0) {
    for (const satir of sonuc) {
      ...
      if (toplamIzinGun > 0) {
        const dusulecekIzinHafta = Math.round(toplamIzinGun / 6)
