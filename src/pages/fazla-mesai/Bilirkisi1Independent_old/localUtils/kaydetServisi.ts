...
export async function kaydetHesap(
  ...
  try {
    ...
    console.log('📤 [KAYDET_SERVISI] Sending payload to API:', JSON.stringify(payload, null, 2));
    console.log('📤 [KAYDET_SERVISI] dataPayload.form.rows:', dataPayload.form?.rows?.length || 0, 'rows');
    if (dataPayload.form?.rows && dataPayload.form.rows.length > 0) {
      console.log('📤 [KAYDET_SERVISI] First row:', dataPayload.form.rows[0]);
    }
