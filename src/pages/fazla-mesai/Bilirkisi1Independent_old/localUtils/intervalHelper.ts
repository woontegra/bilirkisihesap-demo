...
function applyClaimantHours(
  ...
  // TANIK + DAVACI SAAT KESİŞİMİ
  // girisSaat = MAX(davaci.in, tanik.in)
  // cikisSaat = MIN(davaci.out, tanik.out)
  
  const witnessIn = interval.in || '';
  const witnessOut = interval.out || '';
  const claimantIn = claimant.startTime || '';
  const claimantOut = claimant.endTime || '';
  
  // Saat karşılaştırması için normalize et
  ...
  const witnessInMins = timeToMinutes(witnessIn);
  const witnessOutMins = timeToMinutes(witnessOut);
  const claimantInMins = timeToMinutes(claimantIn);
  const claimantOutMins = timeToMinutes(claimantOut);
  
  // KESİŞİM: MAX(in), MIN(out)
  const intersectedInMins = Math.max(witnessInMins, claimantInMins);
  const intersectedOutMins = Math.min(witnessOutMins, claimantOutMins);
  
  // Eğer kesişim yoksa (giriş >= çıkış), boş döndür
  const intersectedIn = minutesToTime(intersectedInMins);
  const intersectedOut = minutesToTime(intersectedOutMins);
  
  return {
    start: toISODateUTC2(interval.start),
    end: toISODateUTC2(interval.end),
    start_time: intersectedIn,
    end_time: intersectedOut,
...
function calculateOneInterval(start: Date, end: Date, w: WitnessInterval): WorkInterval {
  const daily = timeDiffInHours(w.startTime, w.endTime) - 1;
  const weekly = daily * 6;
...
export function calculateIntervals(
  ...
  periods.forEach((p) => {
    const dailyHours = timeDiffInHours(p.giris, p.cikis) - 1;
