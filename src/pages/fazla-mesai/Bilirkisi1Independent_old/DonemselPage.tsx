...
function FazlaMesaiBilirkisi1PageContent({ titleOverride }: Props) {
  ...
  const derivedRows = useMemo(() => {
    ...
    const summerPattern = donemselState.summerPattern || { months: [6, 7, 8], workDays: 6, startTime: "07:00", endTime: "18:00" };
    const winterPattern = donemselState.winterPattern || { months: [12, 1, 2], workDays: 6, startTime: "08:00", endTime: "17:00" };
    
    // Doğrulama: Tüm aylar seçilmiş mi?
    const totalMonths = summerPattern.months.length + winterPattern.months.length;
    if (totalMonths !== 12) {
      console.warn('⚠️ [DONEMSEL] Tüm aylar seçilmedi:', totalMonths, '/12');
      return [];
    }
    ...
    // Basit çözüm: Sadece davacı dönemini kullan (tanık kesişimi yok)
    // TODO: Tanık kesişimi mantığı eklenecek
    const intervals = [{
      start: dateIn,
      end: dateOut,
    ...
    console.log('📊 [DONEMSEL] Intervals (sadece davacı):', intervals.length);
    ...
    intervals.forEach((interval, intervalIdx) => {
      ...
      segments.forEach((seg) => {
        ...
        const fmHours = Math.max(0, weeklyTotal - 45);
  ...
  }, [calculationScenario, donemselState, standardState.davaci, standardState.taniklar, katSayi, weeklyDays]);
  ...
  const activeRows = useMemo(() => {
    ...
    if (calculationScenario === 'DONEMSEL') {
      // DONEMSEL: derivedRows'tan direkt al (exclusions uygulanmış)
      return derivedRowsWithExclusions;
    }
    return [];
  }, [calculationScenario, standartFinalRows, haftalikKarmaFinalRows, derivedRowsWithExclusions]);
