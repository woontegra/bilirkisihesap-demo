...
function DonemselPageContent({ titleOverride }: Props) {
  ...
  const videoLink = getVideoLink("fazla-donemsel");
      ...
      try {
        ...
        if (result) {
          ...
          if (caseId && !effectiveId) {
            ...
            navigate(`/fazla-mesai/donemsel/${caseId}`, { replace: true });
          }
  ...
  const derivedRows = useMemo(() => {
    ...
    intervals.forEach((interval, intervalIdx) => {
      ...
      segments.forEach((seg) => {
        ...
        // FM saatini hesapla (pattern'a göre)
        ...
        const workDays = Number(weeklyDays) || 6; // Üstteki "Haftada Çalışılan Gün" inputundan al
  ...
  const claimantTextCalculation = useMemo(() => {
    ...
    const summerStartMinutes = summerStartH * 60 + summerStartM;
    const summerEndMinutes = summerEndH * 60 + summerEndM;
    const summerDailyMinutes = summerEndMinutes - summerStartMinutes;
    const summerDailyBrut = summerDailyMinutes / 60;
    ...
    const summerWeeklyTotal = Math.round(summerDailyNet * workDays);
    const summerFmHours = Math.max(0, summerWeeklyTotal - 45);

    // Kış dönemi hesaplama
    ...
    const winterStartMinutes = winterStartH * 60 + winterStartM;
    const winterEndMinutes = winterEndH * 60 + winterEndM;
    const winterDailyMinutes = winterEndMinutes - winterStartMinutes;
    const winterDailyBrut = winterDailyMinutes / 60;
    ...
    const winterWeeklyTotal = Math.round(winterDailyNet * workDays);
    const winterFmHours = Math.max(0, winterWeeklyTotal - 45);

    // Format helper
    const fmt = (n: number) => n.toFixed(2).replace('.', ',');

    // SABİT FORMAT - DİNAMİK VERİ (BASIT - HAFTA TATİLSİZ)
    const text = `DAVACI:

Yaz : ${summerMonths} ${summerPattern.startTime} - ${summerPattern.endTime} = ${fmt(summerDailyBrut)} saat çalışma ${fmt(summerBreakHours)} saat ara dinlenme = ${fmt(summerDailyNet)} saat,
...
Kış : ${winterMonths} ${winterPattern.startTime} - ${winterPattern.endTime} = ${fmt(winterDailyBrut)} saat çalışma ${fmt(winterBreakHours)} saat ara dinlenme = ${fmt(winterDailyNet)} saat,
...
${fmt(winterWeeklyTotal)} – 45 saat yasal haftalık çalışma = ${fmt(winterFmHours)} saat haftalık fazla mesai`;
  ...
  const activeRows = useMemo(() => {
    ...
    if (calculationScenario === 'DONEMSEL') {
      return donemselFinalRows;
    }
    return [];
  }, [calculationScenario, standartFinalRows, haftalikKarmaFinalRows, donemselFinalRows]);

  // finalRows alias; setFinalRows satır ekleme/silme ve hücre düzenlemesi için
  const finalRows = activeRows;
  const setFinalRows = useCallback((rows: any[] | ((prev: any[]) => any[])) => {
  ...
  // SYNC: derivedRowsWithExclusions → DONEMSEL finalRows (tablo verisi + satır ekle/sil için)
  useEffect(() => {
    if (!location.pathname.includes('/donemsel') || location.pathname.includes('donemsel-karma') || location.pathname.includes('donemsel-haftalik')) return;
    if (calculationScenario === 'DONEMSEL') {
  ...
  const handleNewCalculation = () => {
    try {
      ...
      navigate("/fazla-mesai/donemsel", { replace: true });
  ...
  const handleSave = () => {
    ...
    kaydetAc({
      hesapTuru: "donemsel_fazla_mesai",
      veri: {
        data: {
          form: {
      ...
      redirectPath: "/fazla-mesai/donemsel/:id",
    });
  };
  ...
  const fmPeriods = useMemo(() => {
    ...
    if (calculationScenario === "DONEMSEL") {
      ...
      if (Number(weeklyDays) === 7 && activeTab === 'tatilli') {
        ...
        // Yaz dönemi hafta tatili hesabı (tek yerde yuvarlama)
        ...
        // Kış dönemi hafta tatili hesabı (tek yerde yuvarlama)
...
${roundedWinterWeekly} – 45 saat yasal haftalık çalışma = ${winterFmStandart} saat haftalık fazla mesai`;
      ...
      witnessesSeasons.forEach((witness: any, idx: number) => {
        ...
        const witnessSummerPattern = witness.summerPattern || { months: [6, 7, 8], startTime: "07:00", endTime: "18:00" };
        const witnessWinterPattern = witness.winterPattern || { months: [12, 1, 2], startTime: "08:00", endTime: "17:00" };
        ...
        // Tanığın çalışma saatlerini hesapla
        ...
        if (Number(weeklyDays) === 7 && activeTab === 'tatilli') {
          // HAFTA TATİLLİ MODU - HAFTA TATİLİ FAZLA MESAİSİ DAHİL (tek yerde yuvarlama)
          const haftaTatiliYasal = 7.5;
          
          // Yaz dönemi hafta tatili hesabı
          const summer6GunToplam = witnessSummerWork.workHours * 6;
          const summerHtFm = Math.max(0, witnessSummerWork.workHours - haftaTatiliYasal);
          const summer7GunToplam = summer6GunToplam + summerHtFm;
          const roundedSummer7 = Math.round(summer7GunToplam);
          const summerFmWith7Days = Math.max(0, roundedSummer7 - 45);
          
          // Kış dönemi hafta tatili hesabı
          const winter6GunToplam = witnessWinterWork.workHours * 6;
          const winterHtFm = Math.max(0, witnessWinterWork.workHours - haftaTatiliYasal);
          const winter7GunToplam = winter6GunToplam + winterHtFm;
          const roundedWinter7 = Math.round(winter7GunToplam);
          const winterFmWith7Days = Math.max(0, roundedWinter7 - 45);
...
Yaz : ${witnessSummerMonthNames} ${witnessSummerPattern.startTime} - ${witnessSummerPattern.endTime} = ${fmt(witnessSummerWork.totalHours)} saat çalışma ${fmt(witnessSummerWork.breakHours)} saat ara dinlenme = ${fmt(witnessSummerWork.workHours)} saat,
6 gün X ${fmt(witnessSummerWork.workHours)} saat = ${fmt(summer6GunToplam)} saat
${fmt(witnessSummerWork.workHours)} - ${fmt(haftaTatiliYasal)} saat (hafta tatili) = ${fmt(summerHtFm)} saat hafta tatili fazla mesai,
${fmt(summer6GunToplam)} saat + ${fmt(summerHtFm)} saat (hafta tatili) = ${fmt(summer7GunToplam)} saat
Net haftalık çalışma = ${roundedSummer7} saat,
${roundedSummer7} – 45 saat yasal haftalık çalışma = ${summerFmWith7Days} saat haftalık fazla mesai

Kış : ${witnessWinterMonthNames} ${witnessWinterPattern.startTime} - ${witnessWinterPattern.endTime} = ${fmt(witnessWinterWork.totalHours)} saat çalışma ${fmt(witnessWinterWork.breakHours)} saat ara dinlenme = ${fmt(witnessWinterWork.workHours)} saat,
6 gün X ${fmt(witnessWinterWork.workHours)} saat = ${fmt(winter6GunToplam)} saat
${fmt(witnessWinterWork.workHours)} - ${fmt(haftaTatiliYasal)} saat (hafta tatili) = ${fmt(winterHtFm)} saat hafta tatili fazla mesai,
${fmt(winter6GunToplam)} saat + ${fmt(winterHtFm)} saat (hafta tatili) = ${fmt(winter7GunToplam)} saat
Net haftalık çalışma = ${roundedWinter7} saat,
${roundedWinter7} – 45 saat yasal haftalık çalışma = ${winterFmWith7Days} saat haftalık fazla mesai`;
        } else {
          // HAFTA TATİLSİZ MODU - STANDART (tek yerde yuvarlama)
          const witnessName = witness.name || `Tanık ${idx + 1}`;
          const swRaw = witnessSummerWork.workHours * Number(weeklyDays);
          const roundedSummerW = Math.round(swRaw);
          const summerFmW = Math.max(0, roundedSummerW - 45);
          const wwRaw = witnessWinterWork.workHours * Number(weeklyDays);
          const roundedWinterW = Math.round(wwRaw);
          const winterFmW = Math.max(0, roundedWinterW - 45);
          witnessText = `${witnessName.toUpperCase()}:

Yaz : ${witnessSummerMonthNames} ${witnessSummerPattern.startTime} - ${witnessSummerPattern.endTime} = ${fmt(witnessSummerWork.totalHours)} saat çalışma ${fmt(witnessSummerWork.breakHours)} saat ara dinlenme = ${fmt(witnessSummerWork.workHours)} saat,
${weeklyDays} gün X ${fmt(witnessSummerWork.workHours)} saat = ${fmt(swRaw)} saat
Net haftalık çalışma = ${roundedSummerW} saat,
${roundedSummerW} – 45 saat yasal haftalık çalışma = ${summerFmW} saat haftalık fazla mesai

Kış : ${witnessWinterMonthNames} ${witnessWinterPattern.startTime} - ${witnessWinterPattern.endTime} = ${fmt(witnessWinterWork.totalHours)} saat çalışma ${fmt(witnessWinterWork.breakHours)} saat ara dinlenme = ${fmt(witnessWinterWork.workHours)} saat,
${weeklyDays} gün X ${fmt(witnessWinterWork.workHours)} saat = ${fmt(wwRaw)} saat
Net haftalık çalışma = ${roundedWinterW} saat,
${roundedWinterW} – 45 saat yasal haftalık çalışma = ${winterFmW} saat haftalık fazla mesai`;
        }
    ...
    >
      ...
      <FooterActions
        ...
        previewButton={{
          ...
          copyTargetId: "donemsel-word-copy",
          ...
          renderContent: () => (
            <div style={{ background: "white", padding: 24 }}>
              <style>{`
                ...
                #donemsel-word-copy table { border-collapse: collapse; width: 100%; margin-bottom: 12px; border: 1px solid #999; font-size: 9px; }
                #donemsel-word-copy td { border: 1px solid #999; padding: 4px 6px; }
              `}</style>
              <div id="donemsel-word-copy">
