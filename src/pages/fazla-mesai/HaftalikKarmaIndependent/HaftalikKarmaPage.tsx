...
import { useHaftalikKarmaScenario } from "./scenarios/HaftalikKarmaScenario";
import { useStandartScenario } from "./scenarios/StandartScenario";
...
// Bolt tasarım stili (Standart FM / Tanıklı / Kıdem ile aynı)
...
export default function HaftalikKarmaPage({ titleOverride }: Props) {
  return (
    <ToastProvider>
      <HaftalikKarmaPageContent titleOverride={titleOverride} />
    </ToastProvider>
  );
}

function HaftalikKarmaPageContent({ titleOverride }: Props) {
  ...
  // HAFTALIK_KARMA PAGE - Scenario is hardcoded to HAFTALIK_KARMA
  useEffect(() => {
    console.log('🔄 [HAFTALIK-KARMA] Setting scenario to HAFTALIK_KARMA');
    setCalculationScenario('HAFTALIK_KARMA');
  }, [setCalculationScenario, location.pathname]);
  
  // Page title - Haftalık Karma sayfası
  const pageTitle = "Haftalık Karma Fazla Mesai Hesaplama";
  ...
  const handleZamanasimiApply = useCallback(() => {
    try {
      ...
      const pandemiBaslangic = new Date('2020-03-13');
      const pandemiBitis = new Date('2020-06-15');
      const effectiveIseGiris = iseGiris || haftalikKarmaState.weeklyStartDateISO;
  ...
  }, [zForm, iseGiris, haftalikKarmaState.weeklyStartDateISO, setZamanasimi, setZamanasimiBaslangic, setShowZamanaModal]);
      ...
      try {
        ...
        if (result) {
          ...
          if (caseId && !effectiveId) {
            ...
            navigate(`/fazla-mesai/haftalik-karma/${caseId}`, { replace: true });
          }
  ...
  // SCENARIO-SPECIFIC FINAL ROWS - HAFTALIK_KARMA PAGE: Only haftalikKarmaFinalRows
  const [haftalikKarmaFinalRows, setHaftalikKarmaFinalRows] = useState<any[]>([]);
  
  // FM SYNC TRIGGER - fmHoursMap hazır olduğunda derivedRowsWithSyncedFM'i tetikle
  const [fmSyncTrigger, setFmSyncTrigger] = useState(0);
  ...
  // ACTIVE ROWS - HAFTALIK_KARMA PAGE: Always use haftalikKarmaFinalRows
  const activeRows = useMemo(() => {
    console.log('[ACTIVE ROWS] haftalikKarmaFinalRows length:', haftalikKarmaFinalRows.length);
    return haftalikKarmaFinalRows;
  }, [haftalikKarmaFinalRows]);

  // finalRows alias for backward compatibility
  const finalRows = activeRows;
  
  console.log('[FINAL ROWS] finalRows length:', finalRows.length);
  ...
  // Artık tüm hesaplama derivedRows useMemo içinde yapılıyor (StandartPage.tsx gibi)

  // DERIVED ROWS - HAFTALIK_KARMA PAGE: Generate rows from witness intersections
  const derivedRows = useMemo(() => {
    try {
      console.log('[HAFTALIK_KARMA CALC] Starting derivedRows calculation');
      
      const dateIn = haftalikKarmaState.weeklyStartDateISO;
      const dateOut = haftalikKarmaState.weeklyEndDateISO;
    
    // HaftalikKarma için dayGroups'tan saatleri al
    const dayGroups = haftalikKarmaState.dayGroups || [];
    if (dayGroups.length === 0) {
      console.log('[HAFTALIK_KARMA CALC] No day groups defined');
      return [];
    }
    
    // İlk grup'un saatlerini kullan (basitleştirme - daha sonra grup bazlı hesaplama eklenebilir)
    const firstGroup = dayGroups[0];
    const timeIn = firstGroup?.startTime || '07:00';
    const timeOut = firstGroup?.endTime || '18:00';
    
    if (!dateIn || !dateOut || !timeIn || !timeOut) {
      console.log('[HAFTALIK_KARMA CALC] Eksik davacı verisi');
      return [];
    }
    ...
    console.log('[HAFTALIK_KARMA CALC] Davacı objesi:', davaci);
    
    // Tanıkları al
    const witnesses = haftalikKarmaState.witnesses || [];
    
    // TANIK OVERLAP SPLIT - Tanık varsa
    // HaftalikKarma tanıkları farklı formatta: { startDateISO, endDateISO, dayGroups: [{ startTime, endTime }] }
    // Standart formata çevir: { dateIn, dateOut, in, out }
    const allWitnesses = witnesses.length > 0 ? witnesses.map((w: any) => {
      // İlk dayGroup'un saatlerini kullan (basitleştirme)
      const firstGroup = w.dayGroups?.[0] || { startTime: '07:00', endTime: '18:00' };
      return {
        dateIn: w.startDateISO || w.dateIn,
        dateOut: w.endDateISO || w.dateOut,
        in: firstGroup.startTime || w.in || '07:00',
        out: firstGroup.endTime || w.out || '18:00'
      };
    }) : [];
    
    const sortedWitnesses = [...allWitnesses].sort((a, b) => {
      const aStart = new Date(a.dateIn);
      const bStart = new Date(b.dateIn);
    ...
    // DAVACI BEYANLARI + TANIK KESİŞİMLERİ: generateDynamicIntervalsFromWitnesses ile tek kronolojik interval listesi
    // Önce davacı sonra tanıklar değil — kesişim uygulanır, her interval için FM saati ilgili kişiden (metin hesaplaması)
    ...
    const fmHoursMap = (window as any).__haftalikKarmaFMHours as Map<string, number> | undefined;
    // Kesişim döneminde hangi kişinin FM'i kullanılacak: tanıklar önce (kesişim = tanık dönemi), sonra DAVACI
    const allPeriodsForLabel: Array<{ start: Date; end: Date; label: string }> = [];
    if (haftalikKarmaState.witnesses && haftalikKarmaState.witnesses.length > 0) {
      haftalikKarmaState.witnesses.forEach((w: any, idx: number) => {
        if (w.startDateISO && w.endDateISO) {
          const witnessLabel = (w.name && String(w.name).trim()) || `TANIK ${idx + 1}`;
          allPeriodsForLabel.push({
    ...
    allPeriodsForLabel.push({ start: new Date(dateIn), end: new Date(dateOut), label: 'DAVACI' });

    const getPeriodLabelForInterval = (intervalStart: Date, intervalEnd: Date): string => {
      for (const p of allPeriodsForLabel) {
        if (p.start.getTime() <= intervalStart.getTime() && p.end.getTime() >= intervalEnd.getTime()) return p.label;
      }
      return 'DAVACI';
    };
    if (fmHoursMap && fmHoursMap instanceof Map && fmHoursMap.has('DAVACI')) {
      witnessIntersectionFMRef.current = fmHoursMap.get('DAVACI') ?? 0;
    }
    ...
    intervals.forEach((interval: any) => {
      ...
      const periodLabel = getPeriodLabelForInterval(intervalStart, intervalEnd);
      const periodWeeklyFMHours = (fmHoursMap && fmHoursMap instanceof Map && fmHoursMap.get(periodLabel)) ?? 0;
      const segments = splitByAsgariUcretPeriods(intervalStart, intervalEnd);
      segments.forEach((seg: { start: Date; end: Date }) => {
        const rowStart = seg.start;
        const rowEnd = seg.end;
    ...
    console.log('[HAFTALIK_KARMA CALC] Generated rows (davacı + tanık kesişimi, FM from text):', generatedRows.length);
    
    // 270 DÜŞÜM
    console.log('[270 DÜŞÜM] include270:', include270, 'mode270:', mode270);
    if (include270) {
      if (mode270 === 'simple') {
        console.log('[270 DÜŞÜM] Applying YARGITAY deduction (5.2 hours per week)');
        ...
        generatedRows = generatedRows.map((row) => {
          ...
          console.log(`[270 DÜŞÜM] Row FM: ${row.fmHours} → ${adjustedFmHours}`);
          return {
            ...row,
        ...
        console.log('[270 DÜŞÜM] Applying ŞİRKET deduction (calculateOvertimeWith270AndLimitation)');
        ...
        generatedRows = generatedRows.map((row) => {
          ...
          const newWeeks = sonuclar[0]?.fmHafta || 0;
    ...
    // FM VE NET HESAPLA (270 düşümü uygulandıktan sonra)
    generatedRows = generatedRows.map(row => {
    ...
    console.log('[HAFTALIK_KARMA CALC] 270 düşüm sonrası örnek satır:', generatedRows[0]);
    
    console.log('[HAFTALIK_KARMA CALC] Final rows:', generatedRows.length);
    
    return generatedRows;
    } catch (error) {
      console.error('[HAFTALIK_KARMA CALC] ERROR:', error);
      return [];
    }
  }, [
    ...
    haftalikKarmaState.weeklyHolidayGroup,
    weeklyDays,
    katSayi,
    zamanasimiBaslangic,
    include270,
  ...
  const derivedRowsOLD = useMemo(() => {
    ...
    // Artık sadece yeni derivedRows useMemo kullanılıyor
    return [];
    ...
    switch (calculationScenario) {
      case "STANDART_OLD": {
        ...
        intervals.forEach((interval, intervalIdx) => {
          ...
          if (workDays === 7 && activeTab === 'tatilli') {
            // HAFTA TATİLLİ HESAPLAMA (tek yerde yuvarlama)
            ...
            // HAFTA TATİLSİZ HESAPLAMA (tek yerde yuvarlama)
        ...
        if (include270 && mode270 === 'simple') {
          ...
          // HAFTALIK_KARMA için doğru tarih aralığını kullan
          const iseGirisTarihi = new Date(dateIn);
          const istenCikisTarihi = new Date(dateOut);
      ...
      case "HAFTALIK_KARMA": {
          ...
          return generatedRows
            .map(row => {
              ...
              if (rowStartDate < zamanasimiDate && rowEndDate >= zamanasimiDate) {
                ...
                // KRİTİK: 270 düşümü zaten row.weeks'te uygulanmış
                // Oransal olarak düşür: (adjustedWeeks / originalWeeks) * row.weeks
                const originalWeeks = row.originalWeekCount || row.weeks;
                const ratio = adjustedWeeks / originalWeeks;
                const finalWeeks = Math.max(0, Math.round(row.weeks * ratio));
                
                const adjustedFm = finalWeeks * row.brut * row.katsayi * row.fmHours / 225 * 1.5;
        ...
        // 6️⃣ KATSAYI UYGULAMA + FINAL FM HESAPLAMA
        const haftalikKarmaKatsayi = katSayi || 1;
        console.log('🔢 [HAFTALIK_KARMA] Katsayı uygulanıyor:', haftalikKarmaKatsayi);
        generatedRows = generatedRows.map(row => {
          // FM tutarı hesapla: weeks × brut × katsayi × fmHours / 225 × 1.5
          const step1 = row.weeks * row.brut;
          const step2 = step1 * haftalikKarmaKatsayi;
          const step3 = step2 * row.fmHours;
          const step4 = step3 / 225;
          const step5 = step4 * 1.5;
          const fm = Number(step5.toFixed(2));
          ...
          return {
            ...row,
            katsayi: haftalikKarmaKatsayi,
            fm: fm,
            net: net,
          };
        });
        
        console.log('🔢 [HAFTALIK_KARMA] Katsayı uygulandıktan sonra örnek satır:', generatedRows[0]);
  ...
  // derivedRows'daki FM saatlerini fmPeriods'dan gelen FM saatleriyle senkronize et
  const derivedRowsWithSyncedFM = useMemo(() => {
    try {
      if (!derivedRows || derivedRows.length === 0) return derivedRows || [];
      
      // fmHoursMap'ten FM saatlerini al
      const fmHoursMap = (window as any).__haftalikKarmaFMHours as Map<string, number>;
      if (!fmHoursMap || !(fmHoursMap instanceof Map)) return derivedRows;
      
      // Her satır için periodLabel'a göre FM saatini güncelle
      return derivedRows.map(row => {
        if (!row) return row;
        
        const periodLabel = row.periodLabel || 'DAVACI';
        let fmHoursFromText = fmHoursMap.get(periodLabel);
        
        if (fmHoursFromText !== undefined && typeof fmHoursFromText === 'number') {
          // 270 DÜŞÜM: Eğer Yargıtay düşümü aktifse, FM saatinden 5.2 saat düş
          if (include270 && mode270 === 'simple') {
            const YARGITAY_270_WEEKLY_DEDUCTION = 5.2;
            fmHoursFromText = Math.max(0, fmHoursFromText - YARGITAY_270_WEEKLY_DEDUCTION);
          }
          
          // Metin hesaplamasından gelen FM saatini kullan (270 düşümü uygulanmış)
          const fm = row.weeks * row.brut * row.katsayi * fmHoursFromText / 225 * 1.5;
          return {
            ...row,
            fmHours: fmHoursFromText,
            fm
          };
        }
      ...
      console.error('[HAFTALIK_KARMA] derivedRowsWithSyncedFM error:', error);
      return derivedRows || [];
    }
  }, [derivedRows, fmSyncTrigger]); // derivedRows ve fmSyncTrigger değiştiğinde yeniden hesapla

  const derivedRowsWithExclusions = useMemo(
    () =>
      applyAnnualLeaveExclusions(derivedRowsWithSyncedFM, exclusions, {
        minWeeks: 0,
      }),
    [derivedRowsWithSyncedFM, exclusions]
  );
  ...
  // fmHoursMap hazır olduğunda derivedRows'u yeniden hesapla
  useEffect(() => {
    const checkAndUpdate = () => {
      const fmHoursMap = (window as any).__haftalikKarmaFMHours as Map<string, number>;
      if (fmHoursMap && fmHoursMap instanceof Map && fmHoursMap.size > 0) {
        console.log('[HAFTALIK_KARMA] fmHoursMap ready, triggering sync. DAVACI FM:', fmHoursMap.get('DAVACI'));
        setFmSyncTrigger(prev => prev + 1);
      }
    };
    
    const timer = setTimeout(checkAndUpdate, 100);
    return () => clearTimeout(timer);
  }, [haftalikKarmaState.weeklyStartDateISO, haftalikKarmaState.weeklyEndDateISO, haftalikKarmaState.dayGroups, haftalikKarmaState.witnesses, haftalikKarmaState.hasWeeklyHoliday, haftalikKarmaState.weeklyHolidayGroup]);

  // SYNC: derivedRowsWithExclusions → HAFTALIK_KARMA finalRows
  // İçerik değişmediyse setState yapma (derivedRowsWithExclusions her .map() ile yeni referans verir → sonsuz döngüyü kır)
  const derivedRowsSyncSignatureRef = useRef<string>('');
  useEffect(() => {
    const next = derivedRowsWithExclusions;
    const len = next?.length ?? 0;
    const sig = len === 0
      ? '0'
      : `${len}|${next[0]?.startISO ?? ''}|${next[0]?.endISO ?? ''}|${next[len - 1]?.startISO ?? ''}|${next[len - 1]?.endISO ?? ''}|${next.reduce((s, r) => s + (r?.fm ?? 0), 0).toFixed(2)}`;
    if (derivedRowsSyncSignatureRef.current === sig) return;
    derivedRowsSyncSignatureRef.current = sig;
    setHaftalikKarmaFinalRows([...next]);
    rowsRef.current = next;
  }, [derivedRowsWithExclusions]);
  
  // Row manipulation functions (after finalRows is defined)
  const duplicateRow = useCallback((i: number) => {
    const updated = [...haftalikKarmaFinalRows];
    // Create NEW EMPTY row (not a copy)
    const newRow = createManualRow();
    // Insert below current row
    updated.splice(i + 1, 0, newRow);
    setHaftalikKarmaFinalRows(updated);
    console.log('[+] Yeni boş satır eklendi, index:', i + 1);
  }, [haftalikKarmaFinalRows, createManualRow]);

  const deleteRow = useCallback((i: number) => {
    if (haftalikKarmaFinalRows.length <= 1) {
      console.warn('[deleteRow] Cannot delete - at least 1 row must remain');
      return;
    }
    const updated = haftalikKarmaFinalRows.filter((_, idx) => idx !== i);
    setHaftalikKarmaFinalRows(updated);
    console.log('[-] Satır silindi, index:', i);
  }, [haftalikKarmaFinalRows]);
  ...
  const handleSave = () => {
    ...
    kaydetAc({
      hesapTuru: "haftalik_karma_fazla_mesai",
      veri: {
        data: {
          form: {
      ...
      redirectPath: "/fazla-mesai/haftalik-karma/:id",
    });
  };
  ...
  // ❌ ESKİ AUTO-CALC USEEFFECT KALDIRILDI
  // Artık tüm hesaplar derivedRows useMemo içinde yapılıyor
  // Tabloya yansıma derivedRowsWithExclusions → haftalikKarmaFinalRows sync ile oluyor
  ...
  // Dinamik metin: HAFTALIK_KARMA PAGE - Only HAFTALIK_KARMA logic
  const fmPeriods = useMemo(() => {
    // HAFTALIK_KARMA: Kendi logic'i ile hesapla
    if (!haftalikKarmaState.weeklyStartDateISO || !haftalikKarmaState.weeklyEndDateISO) {
      ...
      const generateWeeklyText = (dayGroups: any[], title: string, hasWeeklyHoliday: boolean = false, weeklyHolidayGroup: number = 1) => {
        ...
        // İlk satır: İsim (Tanıklı Standart formatı)
        ...
        dayGroups.forEach((group, groupIdx) => {
          ...
          // Hafta tatili kontrolü - seçilen grup için 1 gün hafta tatili, geri kalanı normal
          const isHolidayGroup = hasWeeklyHoliday && (groupIdx + 1) === weeklyHolidayGroup;
          
          if (isHolidayGroup && days > 0) {
            // Bu grup hafta tatili içeriyor - 1 gün hafta tatili, geri kalan günler normal
            const normalDays = days - 1; // 1 gün hafta tatili, geri kalanı normal
            const normalTotal = netDaily * normalDays;
            holidayOvertime = Math.max(0, netDaily - 7.5);
        ...
        if (groupTotals.length === 0 && !hasWeeklyHoliday) {
          return null;
        }
        ...
        // Format: "Toplam çalışma = 38,00 saat + 21,00 saat = 59 saat"
      ...
      // 1) DAVACI metni
      const davaciText = generateWeeklyText(
        haftalikKarmaState.dayGroups, 
        'DAVACI',
        haftalikKarmaState.hasWeeklyHoliday,
        haftalikKarmaState.weeklyHolidayGroup
      ...
      if (davaciText) {
        ...
        fmHoursMap.set('DAVACI', davaciText.weeklyFMHours);
      }
      ...
      if (haftalikKarmaState.witnesses && haftalikKarmaState.witnesses.length > 0) {
        ...
        haftalikKarmaState.witnesses.forEach((witness, idx) => {
          ...
          // GROUP INDEX BAZLI KESİŞİM: Her grup yalnızca kendi karşılığı ile kıyaslanır.
          // finalStart = max(davaci[groupIndex].start, tanik[groupIndex].start)
          // finalEnd   = min(davaci[groupIndex].end,   tanik[groupIndex].end)
          // Eşleşmeyen indexler hesaplanmaz (sadece eşleşen gruplar).
          witnessGroups = witnessGroups.map((group, groupIdx) => {
            const davaciGroup = davaciGroups[groupIdx];
            if (!davaciGroup?.startTime || !davaciGroup?.endTime) {
              return group; // Davacıda bu index yoksa tanık saatlerini olduğu gibi kullan
            }
            ...
            const tGirMinutes = tGirH * 60 + tGirM;
            const tCikMinutes = tCikH * 60 + tCikM;
            const dGirMinutes = dGirH * 60 + dGirM;
            const dCikMinutes = dCikH * 60 + dCikM;
            
            // KESİŞİM (group bazlı): MAX(davacı[groupIdx].start, tanık[groupIdx].start), MIN(davacı[groupIdx].end, tanık[groupIdx].end)
            const kesikGirMinutes = Math.max(tGirMinutes, dGirMinutes);
            const kesikCikMinutes = Math.min(tCikMinutes, dCikMinutes);
            
            const kesikGirH = Math.floor(kesikGirMinutes / 60);
            const kesikGirM = kesikGirMinutes % 60;
            const kesikCikH = Math.floor(kesikCikMinutes / 60);
            const kesikCikM = kesikCikMinutes % 60;
            
            return {
              ...group,
              startTime: `${String(kesikGirH).padStart(2, '0')}:${String(kesikGirM).padStart(2, '0')}`,
              endTime: `${String(kesikCikH).padStart(2, '0')}:${String(kesikCikM).padStart(2, '0')}`
            };
          });
          
          const witnessName = witness.name || `TANIK ${idx + 1}`;
          const witnessText = generateWeeklyText(
            witnessGroups,
            witnessName,
            witness.hasWeeklyHoliday || false,
            witness.weeklyHolidayGroup || 1
          );
          if (witnessText) {
            results.push(witnessText);
            fmHoursMap.set(witnessName, witnessText.weeklyFMHours);
          }
        });
      }
    ...
    >
        ...
        <div className="space-y-4">
          ...
          {showZamanaModal && (
            <ZamanasimiModalContent
              ...
              iseGiris={iseGiris || haftalikKarmaState.weeklyStartDateISO || undefined}
            />
          )}
            {/* ═══════════════════════════════════════════════════════════════════════════ */}
            ...
            {/* Davacı Beyanı - HAFTALIK_KARMA PAGE: Always visible */}
                ...
                <div className="space-y-4">
                  <div className="text-xs text-gray-600 italic">Haftalık Karma Hesaplama</div>
                    ...
                    <div>
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const otherDayCount = newGroups[otherIdx].dayCount || 0;
                                    
                                    // Toplam 7'yi geçemez
                                    if (newValue + otherDayCount > 7) {
                                      newGroups[idx].dayCount = 7 - otherDayCount;
                                    } else {
                                      newGroups[idx].dayCount = newValue;
                                    }
                      ...
                      {/* Hafta Tatili Ayarları - Sadece toplam 7 gün olduğunda göster */}
                      {(() => {
                        const totalDays = haftalikKarmaState.dayGroups.reduce((sum, g) => sum + (g.dayCount || 0), 0);
                        return totalDays === 7 ? (
                          <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded">
                            <div className="flex items-center gap-2 mb-2">
                              <input
                                type="checkbox"
                                id="hasWeeklyHoliday"
                                checked={haftalikKarmaState.hasWeeklyHoliday}
                                onChange={(e) => {
                                  setHaftalikKarmaState(prev => ({ 
                                    ...prev, 
                                    hasWeeklyHoliday: e.target.checked 
                                  }));
                                }}
                                disabled={isReadOnly}
                                className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                              />
                              <label htmlFor="hasWeeklyHoliday" className="text-xs font-semibold text-gray-700 cursor-pointer">
                                Hafta Tatili Var mı?
                              </label>
                            </div>
                            ...
                            {haftalikKarmaState.hasWeeklyHoliday && (
                              <div className="ml-6">
                                ...
                                <select
                                  value={haftalikKarmaState.weeklyHolidayGroup}
                                  onChange={(e) => {
                                    setHaftalikKarmaState(prev => ({ 
                                      ...prev, 
                                      weeklyHolidayGroup: parseInt(e.target.value) 
                                    }));
                                  }}
                                  disabled={isReadOnly}
                                  className={inputClass}
            ...
            {/* Tanık Beyanları - HAFTALIK_KARMA PAGE: Always visible */}
            <details className="rounded-lg border border-gray-200" open>
                  ...
                  {haftalikKarmaState.witnesses.map((witness, idx) => (
                      <div key={witness.id} className="border border-green-200 rounded-lg p-3 space-y-3 bg-green-50">
                        ...
                        <div>
                                ...
                                <div className="flex-1 grid grid-cols-3 gap-2">
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const otherDays = newWitnesses[idx].dayGroups[otherIdx]?.days || 0;
                                      
                                      // Toplam 7'yi geçemez
                                      if (newValue + otherDays > 7) {
                                        newWitnesses[idx].dayGroups[gIdx].days = 7 - otherDays;
                                      } else {
                                        newWitnesses[idx].dayGroups[gIdx].days = newValue;
                                      }
                          ...
                          {/* Hafta Tatili Ayarları - Sadece toplam 7 gün olduğunda göster */}
                          {(() => {
                            const totalDays = witness.dayGroups.reduce((sum, g) => sum + (g.days || 0), 0);
                            return totalDays === 7 ? (
                              <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded">
                                <div className="flex items-center gap-2 mb-2">
                                  <input
                                    type="checkbox"
                                    id={`witnessHoliday-${witness.id}`}
                                    checked={witness.hasWeeklyHoliday || false}
                                    onChange={(e) => {
                                      const newWitnesses = [...haftalikKarmaState.witnesses];
                                      newWitnesses[idx].hasWeeklyHoliday = e.target.checked;
                                      if (!newWitnesses[idx].weeklyHolidayGroup) {
                                        newWitnesses[idx].weeklyHolidayGroup = 1;
                                      }
                                      setHaftalikKarmaState(prev => ({ ...prev, witnesses: newWitnesses }));
                                    }}
                                    disabled={isReadOnly}
                                    className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                                  />
                                  <label htmlFor={`witnessHoliday-${witness.id}`} className="text-xs font-semibold text-gray-700 cursor-pointer">
                                    Hafta Tatili Var mı?
                                  </label>
                                </div>
                                
                                {witness.hasWeeklyHoliday && (
                                  <div className="ml-6">
                                    <label className="block text-xs text-gray-600 mb-1">Hangi Gruba Dahil?</label>
                                    <select
                                      value={witness.weeklyHolidayGroup || 1}
                                      onChange={(e) => {
                                        const newWitnesses = [...haftalikKarmaState.witnesses];
                                        newWitnesses[idx].weeklyHolidayGroup = parseInt(e.target.value);
                                        setHaftalikKarmaState(prev => ({ ...prev, witnesses: newWitnesses }));
                                      }}
                                      disabled={isReadOnly}
                                      className={inputClass}
                    ...
                    <div className="bg-[#f1f3f5] border rounded-md p-3">
                      {fmPeriods.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                          {fmPeriods.map((p, idx) => (
                            <div key={idx} className="p-3 rounded-lg border bg-white shadow-sm leading-relaxed whitespace-pre-line" style={{fontSize: '10pt', fontFamily: '"Times New Roman", Times, serif'}}>
                              {p.text}
                            </div>
                          ))}
                        </div>
              ...
              <table className="w-full text-xs border-[0.5px] border-gray-300" style={{ borderCollapse: 'collapse', tableLayout: 'fixed', width: '100%', lineHeight: '1.2' }}>
                ...
                <tbody>
                  {finalRows.length === 0 ? (
                    ...
                    finalRows.map((r, i) => {
                      // DISPLAY FIX: Show endDate as nextRow.startDate - 1 day (display only, no state mutation)
                      const nextRow = finalRows[i + 1];
                      const displayEndISO = (nextRow && nextRow.startISO) ? (() => {
                        const nextStart = new Date(nextRow.startISO);
                        if (isNaN(nextStart.getTime())) return r.endISO; // Geçersiz tarih
                        nextStart.setDate(nextStart.getDate() - 1);
      ...
      <FooterActions
        ...
        previewButton={{
          ...
          copyTargetId: "haftalik-karma-word-copy",
          ...
          renderContent: () => (
            <div style={{ background: "white", padding: 24 }}>
              <style>{`
                ...
                #haftalik-karma-word-copy table { border-collapse: collapse; width: 100%; margin-bottom: 12px; border: 1px solid #999; font-size: 9px; }
                #haftalik-karma-word-copy td { border: 1px solid #999; padding: 4px 6px; }
              `}</style>
              <div id="haftalik-karma-word-copy">
