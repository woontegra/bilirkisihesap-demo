...
export function useFazlaMesaiBilirkisi1State() {
  ...
  const [haftalikKarmaState, setHaftalikKarmaState] = useState({
    ...
    weeklyHolidayGroup: 1, // 1 veya 2 (hangi gruba dahil)
    ...
    witnesses: [] as Array<{ 
      ...
      weeklyHolidayGroup?: number; // 1 veya 2 (hangi gruba dahil)
    }>,
    exclusions: [] as ExcludedDay[]
  });
  ...
  // Dışlamalar - Global state (UI'da gösterilen, aktif scenario ile senkronize edilir)
  ...
  const [savedExclusionSets, setSavedExclusionSets] = useState<{ id: number; name: string; data: ExcludedDay[]; createdAt: string }[]>([]);
