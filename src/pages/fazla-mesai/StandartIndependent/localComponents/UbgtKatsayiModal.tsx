...
interface UbgtKatsayiModalProps {
  open: boolean;
  onClose: () => void;
...
export default function UbgtKatsayiModal({ open, onClose, onApply }: UbgtKatsayiModalProps) {
  ...
  const result = useMemo(() => {
    ...
    if (!minimum || isNaN(minimum) || isNaN(known)) return 0;
    const katsayi = Number((known / minimum).toFixed(4));
    return katsayi;
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-3">
          <div>
            ...
            <Input type="text" value={bilinenUcret} onChange={(e) => setBilinenUcret(e.target.value)} />
          </div>
          <div>
            <Label className="text-sm text-gray-700 dark:text-gray-300 mb-1 block">Asgari Ücret</Label>
            <Input type="text" value={asgariUcret} onChange={(e) => setAsgariUcret(e.target.value)} />
          </div>
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button onClick={() => { onApply(result); onClose(); }} className="bg-blue-600 hover:bg-blue-700 text-white">Uygula</Button>
        </div>
      </div>
    </div>,
    document.body
