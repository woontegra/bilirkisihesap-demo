...
export default function ProfileInfoPage() {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      // Email'i önce user'dan, sonra localStorage'dan al
      const emailToLoad = user?.email || localStorage.getItem("email");
      
      if (!emailToLoad) {
        // Email yoksa mevcut bilgileri kullan
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/admin/users/email/${encodeURIComponent(emailToLoad)}`, {
          headers: {
        ...
        if (response.ok) {
          ...
          // API'den veri alınamazsa mevcut bilgileri kullan
          const currentUser = JSON.parse(localStorage.getItem("current_user") || "{}");
          setFormData({
            name: user?.name || currentUser.name || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Email'i önce formData'dan, sonra user'dan, son olarak localStorage'dan al
    ...
    try {
      ...
      // Email ile kullanıcıyı güncelle
      const response = await fetch(`${API_BASE_URL}/api/admin/users/email/${encodeURIComponent(emailToUse)}`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        ...
        const errorMessage = errorData.error || errorData.details || "Profil güncellenirken bir hata oluştu";
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-blue-900 dark:text-blue-100 mb-1">
              Profil Resmi Nasıl Yüklenir?
            </h3>
            <p className="text-sm text-blue-700 dark:text-blue-300">
              Profil resminizi yüklemek veya değiştirmek için <strong>sayfanın en üstündeki avatar resminize tıklayın</strong>. 
              Buradan hem resim yükleyebilir hem de mevcut resminizi kaldırabilirsiniz.
            </p>
          </div>
        </div>
      </div>
