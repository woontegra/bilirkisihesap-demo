...
const PRINT_HEADING = "Parça Başı Çalışanlar Kıdem Tazminatı Hesaplama";
const PRINT_TITLE = "Parça Başı Çalışanlar Kıdem Tazminatı Hesaplama";
const RECORD_TYPE = "belirli-sureli";
const REDIRECT_BASE_PATH = "/kidem-tazminati/parca-basi";
...
export default function KidemParcaBasiIndependent() {
  ...
  const videoLink = getVideoLink("kidem-parca-basi");
  ...
  const handleFormChange = useCallback((newValues: any) => {
    console.log('[KidemParcaBasi] handleFormChange - newValues:', newValues);
    console.log('[KidemParcaBasi] handleFormChange - newValues.extras:', newValues.extras);
    ...
    setFormValues(prev => {
      ...
      console.log('[KidemParcaBasi] handleFormChange - prev.extras:', prev.extras);
      console.log('[KidemParcaBasi] handleFormChange - extrasChanged:', extrasChanged);
      
      const updated = { ...prev, ...newValues };
      // Extras'ı özellikle ekle (array referansı değişebilir)
      if (newValues.extras !== undefined) {
        updated.extras = newValues.extras;
        console.log('[KidemParcaBasi] handleFormChange - updated.extras:', updated.extras);
      }
      if (newValues.exitDate) {
        setExitDate(newValues.exitDate);
      }
      console.log('[KidemParcaBasi] handleFormChange - final updated:', updated);
      return updated;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const calculateFromBackend = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/kidem/parca-basi`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleSave = useCallback(() => {
    ...
    console.log('[KidemParcaBasi] handleSave - formValues.extras:', formValues.extras);
    console.log('[KidemParcaBasi] handleSave - formValues:', formValues);
    ...
    console.log('[KidemParcaBasi] handleSave - formDataWithExtras:', formDataWithExtras);
    
    kaydetAc({
      hesapTuru: "kidem_parca_basi",
      veri: {
        data: {
