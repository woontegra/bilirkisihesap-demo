  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-2xl p-6 animate-scaleIn">
        ...
        <div className="mt-4 text-sm text-gray-600">Formül: (aylık toplam / 360) × 30</div>
        <div className="mt-2 text-base font-medium text-gray-900">Sonuç: {new Intl.NumberFormat("tr-TR", { style: "currency", currency: "TRY" }).format(result || 0)}</div>
        <div className="mt-6 flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 rounded-md border border-gray-300 text-sm">İptal</button>
          <button onClick={() => { (onConfirm || onApply)?.(result || 0); onClose(); }} className="px-4 py-2 rounded-md bg-blue-600 hover:bg-blue-700 text-white text-sm">Uygula</button>
        </div>
      </div>
    </div>
  );
