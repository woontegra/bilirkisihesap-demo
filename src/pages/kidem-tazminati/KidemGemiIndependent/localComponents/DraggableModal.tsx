        ...
        >
          <div className="flex items-center gap-2 flex-1 min-w-0" style={{ minWidth: "200px" }}>
            ...
            <div className="flex gap-1 ml-2 shrink-0">
              <button type="button" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); toggleMinimize(); }} className="w-6 h-6 rounded hover:bg-gray-200 dark:hover:bg-gray-700 flex items-center justify-center text-gray-600 dark:text-gray-400">{isMinimized ? "+" : "-"}</button>
              <button type="button" onMouseDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); toggleMaximize(); }} className="w-6 h-6 rounded hover:bg-gray-200 dark:hover:bg-gray-700 flex items-center justify-center text-gray-600 dark:text-gray-400">[]</button>
            </div>
          </div>
          <div className="modal-header-actions flex items-center gap-2">
            {headerActions}
            <button onClick={onClose} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 text-xl leading-none px-2" title="Kapat">x</button>
          </div>
        </div>
        {!isMinimized && (
