/**
 * ToplamHesaplama.tsx
 * Lokal ToplamHesaplama componenti - SADECE bu sayfa için
 */
...
type Props = {
  ...
  onPrint?: () => void; // Optional, artık kullanılmıyor ama geriye dönük uyumluluk için
};
  ...
  return (
    <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-gray-700 dark:to-gray-800 rounded-2xl p-6 border border-indigo-100 dark:border-gray-600 shadow-sm transition-all" style={{ maxWidth: '100%', boxSizing: 'border-box' }}>
      ...
      <div className="p-4">
        ...
        {warnings?.length > 0 && (
          <div className="mt-2 sm:mt-3 p-2 sm:p-3 rounded bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs sm:text-sm">
            {warnings.map((w, i) => (
              <div key={i}>{w}</div>
            ))}
          </div>
