/**
 * Merkezi Kayıt Hook'u – lokal kopya (bu sayfa için).
 */
...
export function useKaydet() {
  ...
  const performKaydetInternal = useCallback(
    async (kayitAdi: string, options: KaydetOptions) => {
      setIsSaving(true);
      try {
        const result = await kaydetHesap(kayitAdi, options.hesapTuru, options.veri, options.mevcutId);
        setIsModalOpen(false);
        success(result.message || "Kayıt başarıyla kaydedildi");
        if (options.onSuccess) options.onSuccess(result);
        if (options.redirectPath && result.id) {
          const redirectUrl = options.redirectPath.includes(":id") ? options.redirectPath.replace(":id", String(result.id)) : `${options.redirectPath}/${result.id}`;
          setTimeout(() => navigate(redirectUrl, { replace: false }), 300);
        }
      } catch (err: any) {
  ...
  return { kaydetAc, kaydetKapat, isModalOpen, isSaving, KaydetModal: KaydetModalComponent };
}
