...
export default function ToplamHesaplama({ toplam, yil, ay, gun, warnings = [], customFormatter }: Props) {
  ...
  return (
    <div className="bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-gray-700 dark:to-gray-800 rounded-2xl border border-purple-200 dark:border-gray-600 shadow-sm overflow-hidden" style={{ maxWidth: "100%", boxSizing: "border-box" }}>
      <div className="border-b border-purple-100 dark:border-gray-600 px-6 py-4">
        <h3 className="text-sm md:text-base font-bold text-purple-900 dark:text-purple-200 flex items-center gap-2" style={{ wordBreak: "break-word" }}>
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
          </svg>
          Kıdem Tazminatı Hesaplaması
        </h3>
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5" style={{ wordBreak: "break-word" }}>Detaylı hesaplama sonuçları</p>
      </div>
      <div className="p-6 space-y-2">
        <div className="flex items-center justify-between py-1.5 border-b border-gray-200 dark:border-gray-600" style={{ minWidth: 0 }}>
        ...
        {warnings?.length > 0 && (
          <div className="mt-2 sm:mt-3 p-2 sm:p-3 rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs sm:text-sm">
            {warnings.map((w, i) => (
              <div key={i}>{w}</div>
            ))}
          </div>
