...
const validateHaksizFesihForm = (form: any) => {
  const errors: string[] = [];
...
export default function HaksizFesihIndependent() {
  ...
  const [odenenTutar, setOdenenTutar] = useState(""); // İşçiye ödenen tutar (mahsup için)
  ...
  // Hesaplanan değerler (inline - basit katsayı hesaplama)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (form.odenenTutar) {
          setOdenenTutar(form.odenenTutar);
        } else if (formData.odenenTutar) {
          setOdenenTutar(formData.odenenTutar);
        }
  ...
  const haksizFesihReportConfig = useMemo((): ReportConfig => {
    ...
    const odenenTutarVal = parseNum(odenenTutar);
    const mahsupSonrasiNet = Math.max(0, netTotal - odenenTutarVal);

    const grossToNetRows = [
      ...
      { label: "Net Haksız Fesih Tazminatı", value: `${fmtLocal(netTotal)} ₺`, isNet: true },
    ];

    // Mahsup varsa ekle
    if (odenenTutarVal > 0) {
      grossToNetRows.push(
        { label: "İşçiye Ödenen Tutar (Mahsup)", value: `-${fmtLocal(odenenTutarVal)} ₺`, isDeduction: true },
    ...
    return {
      ...
      infoRows: [
        ...
        { label: "İşçiye Ödenen Tutar", value: odenenTutarVal > 0 ? `${fmtLocal(odenenTutarVal)} ₺` : "-", condition: odenenTutarVal > 0 },
      ],
  ...
  }, [brutVal, startDate, endDate, workPeriod, rows, brutForNetConversion, odenenTutar]);

  const wordTableSections = useMemo(() => {
    const sections: Array<{ id: string; title: string; html: string }> = [];

    const infoRowsFiltered = (haksizFesihReportConfig.infoRows || []).filter((r) => r.condition !== false);
    ...
    const pd = haksizFesihReportConfig.periodData;
    if (pd?.rows?.length) {
      const n2 = adaptToWordTable({ headers: pd.headers, rows: pd.rows });
      sections.push({ id: "haksiz-fesih-hesaplama", title: pd.title || "Haksız Fesih Tazminatı Hesaplama Detayı", html: buildWordTable(n2.headers, n2.rows) });
    }

    const gnd = haksizFesihReportConfig.grossToNetData?.rows;
    if (gnd?.length) {
      const n3 = adaptToWordTable(gnd);
      sections.push({ id: "brutten-nete", title: haksizFesihReportConfig.grossToNetData?.title || "Brüt'ten Net'e Çeviri", html: buildWordTable(n3.headers, n3.rows) });
    }

    return sections;
  }, [haksizFesihReportConfig]);

  const handlePrint = useCallback(() => {
    const targetEl = document.getElementById("haksiz-fesih-print-wrapper");
    if (!targetEl) {
      window.print();
      return;
    }
    const title = haksizFesihReportConfig.title;
  ...
  }, [haksizFesihReportConfig.title]);

  const handleSave = () => {
    try {
      const validation = validateHaksizFesihForm({ brut });
      ...
      kaydetAc({
        hesapTuru: "haksiz_fesih_tazminati",
        veri: {
        ...
        redirectPath: `/haksiz-fesih-tazminati/:id`,
      });
    } catch (e) {
  ...
  const handleNewCalculation = () => {
    try {
      ...
      if (id) {
        window.location.href = "/haksiz-fesih-tazminati";
      }
    } catch {}
  };
  ...
  return (
    <Layout
      title={PAGE_TITLE}
      description="Haksız Fesih Tazminatı Hesaplama"
      hideHeader={true}
      fluid={true}
    ...
    >
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
        ...
        <div id="haksiz-fesih-print" className="w-full space-y-6">
          {/* Sol: Hesaplama */}
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl dark:shadow-gray-900/50 border border-gray-100 dark:border-gray-700 overflow-hidden p-6">
            <div className="space-y-3 w-full">
              ...
              <div className="mt-4">
                ...
                />
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  İşçiye ödenen tutar varsa buraya giriniz. Bu tutar net tazminattan mahsup edilecektir.
                </div>
                {odenenTutar && parseNum(odenenTutar) > 0 && (
                  <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
                    <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">Mahsup Sonrası Net Tutar:</div>
                    <div className="text-sm font-semibold text-blue-700 dark:text-blue-300">
                      {fmt(Math.max(0, brutForNetConversion * (1 - 0.00759) - parseNum(odenenTutar)))} ₺
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl dark:shadow-gray-900/50 border border-gray-100 dark:border-gray-700 overflow-hidden p-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between py-2 border-b border-gray-200 dark:border-gray-600">
                <span className="text-sm text-gray-600 dark:text-gray-400">Brüt Haksız Fesih Tazminatı</span>
              ...
              <div className="flex items-center justify-between py-2 border-b border-gray-200 dark:border-gray-600">
                <span className="text-base font-semibold text-green-700 dark:text-green-400">Net Haksız Fesih Tazminatı</span>
                <span className="text-lg font-bold text-green-700 dark:text-green-400">{fmt(brutForNetConversion * (1 - 0.00759))} ₺</span>
              </div>
              {odenenTutar && parseNum(odenenTutar) > 0 && (
                <>
                  <div className="flex items-center justify-between py-2 border-b border-gray-200 dark:border-gray-600">
                    <span className="text-sm text-red-600 dark:text-red-400">İşçiye Ödenen Tutar (Mahsup)</span>
                    <span className="font-semibold text-red-600 dark:text-red-400">-{fmt(parseNum(odenenTutar))} ₺</span>
                  </div>
                  <div className="flex items-center justify-between pt-2">
                    <span className="text-base font-semibold text-blue-700 dark:text-blue-400">Mahsup Sonrası Net Tutar</span>
                    <span className="text-lg font-bold text-blue-700 dark:text-blue-400">{fmt(Math.max(0, brutForNetConversion * (1 - 0.00759) - parseNum(odenenTutar)))} ₺</span>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl dark:shadow-gray-900/50 border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div className="p-4 text-sm leading-6 notes-content">
              <div className="text-slate-600 dark:text-slate-300 space-y-2">
                ...
                <p>
                  Hâkim, bütün durum ve koşulları göz önünde tutarak, ayrıca miktarını serbestçe belirleyeceği bir tazminatın işçiye ödenmesine karar verebilir; ancak belirlenecek tazminat miktarı, işçinin altı aylık ücretinden fazla olamaz." Şeklinde düzenlenmiştir.
                </p>
                <hr className="my-2 border-slate-300 dark:border-slate-600" />
                <p className="font-medium text-slate-800 dark:text-slate-200">
                  Haksız fesih tazminatı, yapılan feshin haksız olması sebebiyle işçiye ödenir.
                </p>
                <p className="text-red-600 dark:text-red-400 font-medium">
                  Ancak belirlenecek tazminat, işçinin 6 aylık ücretinden fazla olamaz.
                </p>
                <p>
                  Haksız fesih tazminatı, bakiye ücret tazminatından ayrı bir tazminat türüdür ve bu tazminata ek olarak verilir.
                </p>
              </div>
            </div>
          </div>
      ...
      <div id="haksiz-fesih-print-wrapper" style={{ position: "absolute", left: "-9999px", top: 0, visibility: "hidden", width: "16cm", zIndex: -1 }} aria-hidden="true">
        <ReportContentFromConfig config={haksizFesihReportConfig} />
      </div>
      ...
      <FooterActions
        ...
        previewButton={{
          title: "Haksız Fesih Tazminatı Rapor",
          ...
          onPdf: () => downloadPdfFromDOM("Haksız Fesih Tazminatı Rapor", "report-content"),
        }}
      />
    </Layout>
  );
