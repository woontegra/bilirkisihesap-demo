...
const NOTE_ITEMS: string[] = ["Part time çalışanlarda yıllık izin hakkı orantılı olarak hesaplanır.", "Haftalık çalışma süresi düşük olanlar için özel hesaplama yapılır."];
const SAVE_ENDPOINT = `${API_BASE_URL}/api/saved-cases`;
const SAVE_TYPE = "Yıllık Ücretli İzin";
const DOCUMENT_TITLE = "Mercan Danışmanlık | Part Time Yıllık Ücretli İzin Alacağı";
const PRINT_TITLE = "Part Time Yıllık Ücretli İzin Hesaplama";
const PRINT_HEADING = "Part Time Yıllık Ücretli İzin Hesaplama";
...
export default function YillikIzinPage() {
  ...
  useEffect(() => {
    const calculateFromBackend = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/yillik-izin/part`, {
          method: "POST",
