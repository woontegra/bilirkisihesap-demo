...
const DOCUMENT_TITLE = "Mercan Danışmanlık | Belirli Süreli Yıllık Ücretli İzin Alacağı";
const PRINT_TITLE = "Belirli Süreli Yıllık Ücretli İzin Hesaplama";
const PRINT_HEADING = "Belirli Süreli Yıllık Ücretli İzin Hesaplama";
...
export default function YillikIzinPage() {
  ...
  const videoLink = getVideoLink("yillik-belirli");
  ...
  useEffect(() => {
    const calculateFromBackend = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/yillik-izin/belirli`, {
          method: "POST",
          headers: { 
    ...
    if (iseGiris && istenCikis && diff.yil >= 0) {
      ...
      // Reset calculations when fields are empty
  ...
  const belirliReportConfig = useMemo((): ReportConfig => {
    ...
    return {
      ...
      customSections: [
        {
          title: "Çalışma Dönemleri (Belirli Süreli Sözleşme)",
          content: (
        ...
        {
          title: "Yıllık İzin Hak Edişi (Belirli Süreli Sözleşme)",
          content: (
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '16px' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #e5e7eb' }}>
  ...
  const wordTableSections = useMemo(() => {
    ...
    if (belirliReportConfig.infoRows && belirliReportConfig.infoRows.length > 0) {
      const n1 = adaptToWordTable({
        headers: ["Alan", "Değer"],
        rows: belirliReportConfig.infoRows.map((r) => [r.label, String(r.value ?? "-")]),
      });
    ...
    if (workPeriodsSummary.length > 0) {
      ...
      sections.push({ id: "calisma-donemleri", title: "Çalışma Dönemleri (Belirli Süreli Sözleşme)", html: buildWordTable(n2.headers, n2.rows) });
    }
    ...
    sections.push({ id: "yillik-izin-hak-edisi", title: "Yıllık İzin Hak Edişi (Belirli Süreli Sözleşme)", html: buildWordTable(n3.headers, n3.rows) });

    const gnd = belirliReportConfig.grossToNetData?.rows;
    if (gnd?.length) {
    ...
    const md = belirliReportConfig.mahsuplasmaData;
    if (md?.rows) {
    ...
    return sections;
  }, [belirliReportConfig, workPeriods, breakdown, usedTotal, remainingDays, brutUcret, brutIzin]);

  const handlePrint = useCallback(() => {
    const targetEl = document.getElementById("belirli-print-wrapper");
    if (!targetEl) {
      window.print();
      return;
    }
    const title = belirliReportConfig.title;
  ...
  const handleSave = () => {
    try {
      ...
      kaydetAc({
        ...
        redirectPath: `/yillik-izin/belirli/:id`,
      });
    } catch (e) {
  ...
  const handleNewCalculation = () => {
    try {
      ...
      if (id) {
        window.location.href = "/yillik-izin/belirli";
      }
    } catch {}
  };
  ...
  return (
    ...
    >
      ...
      <div id="belirli-print-wrapper" style={{ position: "absolute", left: "-9999px", top: 0, visibility: "hidden", width: "16cm", zIndex: -1 }} aria-hidden="true">
      ...
      <FooterActions
        ...
        previewButton={{
          title: `Belirli Süreli ${REPORT_TITLE} Rapor`,
          copyTargetId: "belirli-word-copy",
          ...
          renderContent: () => (
            <div style={{ background: "white", padding: 24 }}>
              <style>{`
                ...
                #belirli-word-copy table { border-collapse: collapse; width: 100%; margin-bottom: 12px; border: 1px solid #999; font-size: 9px; }
                #belirli-word-copy td { border: 1px solid #999; padding: 4px 6px; }
              `}</style>
              <div id="belirli-word-copy">
          ...
          onPdf: () => downloadPdfFromDOM(`Belirli Süreli ${REPORT_TITLE} Rapor`, "report-content"),
        }}
      />
