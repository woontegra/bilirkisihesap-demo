...
// Constants - inline (Mevsimlik)
const NOTE_ITEMS: string[] = ["Mevsimlik işçilerin yıllık izin hakları çalışma süresine göre belirlenir.", "Sezon sonunda işten ayrılanlarda izin ücreti ödenmesi gerekir."];
const SAVE_ENDPOINT = `${API_BASE_URL}/api/saved-cases`;
const SAVE_TYPE = "Yıllık Ücretli İzin";
const DOCUMENT_TITLE = "Mercan Danışmanlık | Mevsimlik Yıllık Ücretli İzin Alacağı";
const PRINT_TITLE = "Mevsimlik Yıllık Ücretli İzin Hesaplama";
const PRINT_HEADING = "Mevsimlik Yıllık Ücretli İzin Hesaplama";
...
export default function YillikIzinPage() {
  ...
  useEffect(() => {
    const calculateFromBackend = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/yillik-izin/mevsim`, {
          method: "POST",
          headers: {
  ...
  const mevsimlikReportConfig = useMemo((): ReportConfig => {
  ...
  const wordTableSections = useMemo(() => {
    ...
    if (mevsimlikReportConfig.infoRows && mevsimlikReportConfig.infoRows.length > 0) {
      const n1 = adaptToWordTable({ headers: ["Alan", "Değer"], rows: mevsimlikReportConfig.infoRows.map(r => [r.label, String(r.value ?? "-")]) });
    ...
    if (workPeriodsSummary.length > 0) {
      ...
      sections.push({ id: "calisma-donemleri", title: "Çalışma Dönemleri (Mevsimlik)", html: buildWordTable(n2.headers, n2.rows) });
    }
    ...
    sections.push({ id: "yillik-izin-hak-edisi", title: "Yıllık İzin Hak Edişi (Mevsimlik İşçiler)", html: buildWordTable(n3.headers, n3.rows) });

    const gnd = mevsimlikReportConfig.grossToNetData?.rows;
    if (gnd?.length) {
    ...
    const md = mevsimlikReportConfig.mahsuplasmaData;
    if (md?.rows) {
    ...
    return sections;
  }, [mevsimlikReportConfig, workPeriods, breakdown, usedTotal, remainingDays, brutUcret, brutIzin]);

  const handlePrint = useCallback(() => {
    const targetEl = document.getElementById("mevsimlik-print-wrapper");
    if (!targetEl) {
      window.print();
      return;
    }
    const title = mevsimlikReportConfig.title;
  ...
  const handleNewCalculation = () => {
    try {
      ...
      if (id) {
        window.location.href = "/yillik-izin/mevsim";
      }
    } catch {}
  };
  ...
  return (
    ...
    >
      ...
      <div id="mevsimlik-print-wrapper" style={{ position: "absolute", left: "-9999px", top: 0, visibility: "hidden", width: "16cm", zIndex: -1 }} aria-hidden="true">
        <ReportContentFromConfig config={mevsimlikReportConfig} />
      </div>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl dark:shadow-gray-900/50 border border-gray-200 dark:border-gray-700 p-6 sm:p-8">
              ...
              <div className="border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700/30">
                <div className="w-full flex items-center justify-between px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-300">
                  ...
                  <div className="flex gap-2 items-center">
                    <Button type="button" variant="outline" size="sm" onClick={() => { setExclusionSaveName(""); setShowExclusionSaveModal(true); }} disabled={rows.every(r => !r.start || !r.end)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border-blue-300 dark:border-blue-600 text-blue-700 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30">
                      Kaydet
              ...
              <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-600">
                ...
                <div className="space-y-2 text-slate-600 dark:text-slate-300 text-sm leading-6">
                  {NOTE_ITEMS.map((note, index) => {
                    if (note === "") return <br key={index} />;
                    const isHeading = note.includes("YARGITAY") || note.includes("Esas No") || note.includes("Karar No") || note.includes("Tarihi:") || note.includes("MEVSİMLİK") || note.includes("FİİLİ ÇALIŞMANIN");
                    if (isHeading) {
                      return <p key={index} className="font-semibold text-slate-800 dark:text-slate-200">{note}</p>;
                    }
      ...
      <FooterActions
        ...
        previewButton={{
          title: `Mevsimlik ${REPORT_TITLE} Rapor`,
          copyTargetId: "mevsimlik-word-copy",
          ...
          renderContent: () => (
            <div style={{ background: "white", padding: 24 }}>
              <style>{`
                ...
                #mevsimlik-word-copy table { border-collapse: collapse; width: 100%; margin-bottom: 12px; border: 1px solid #999; font-size: 9px; }
                #mevsimlik-word-copy td { border: 1px solid #999; padding: 4px 6px; }
              `}</style>
              <div id="mevsimlik-word-copy">
