...
// Constants - inline (Toplu)
const NOTE_ITEMS: string[] = ["Toplu sözleşmeye tabi işçilerde yıllık izin hakkı toplu sözleşmeye göre belirlenebilir.", "Toplu sözleşmede düzenleme yoksa genel hükümler uygulanır."];
const SAVE_ENDPOINT = `${API_BASE_URL}/api/saved-cases`;
const SAVE_TYPE = "Yıllık Ücretli İzin";
const DOCUMENT_TITLE = "Mercan Danışmanlık | Toplu Sözleşme Yıllık Ücretli İzin Alacağı";
const PRINT_TITLE = "Toplu Sözleşme Yıllık Ücretli İzin Hesaplama";
const PRINT_HEADING = "Toplu Sözleşme Yıllık Ücretli İzin Hesaplama";
...
export default function YillikIzinPage() {
  ...
  useEffect(() => {
    const calculateFromBackend = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/yillik-izin/toplu`, {
          method: "POST",
