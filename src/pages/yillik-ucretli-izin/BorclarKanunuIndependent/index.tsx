...
// Constants - inline (Borçlar Kanunu)
const NOTE_ITEMS: string[] = ["Borçlar Kanununa tabi iş ilişkilerinde yıllık ücretli izin hakkı yılda en az 4 haftadır.", "Bu hesaplama Borçlar Kanunu kapsamında çalışanlar için yapılmıştır."];
const SAVE_ENDPOINT = `${API_BASE_URL}/api/saved-cases`;
const SAVE_TYPE = "Yıllık Ücretli İzin";
const DOCUMENT_TITLE = "Mercan Danışmanlık | Borçlar Kanunu Yıllık Ücretli İzin Alacağı";
const PRINT_TITLE = "Borçlar Kanunu Yıllık Ücretli İzin Hesaplama";
const PRINT_HEADING = "Borçlar Kanunu Yıllık Ücretli İzin Hesaplama";
...
export default function YillikIzinPage() {
  ...
  const videoLink = getVideoLink("yillik-borclar");
  ...
  useEffect(() => {
    const calculateFromBackend = async () => {
      try {
        const response = await authenticatedFetch(`${API_BASE_URL}/api/yillik-izin/borclar-kanunu`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ years: diff.yil, brutUcret: brutUcret, usedRows: rows, exitYear: selectedYear, is18Or50 })
        });
        ...
        // Only show error if we have valid input
        if (iseGiris && istenCikis && brutUcret && diff.yil > 0) {
    ...
    if (iseGiris && istenCikis && diff.yil >= 0) {
      ...
      // Gerekli alanlar dolmamışsa tüm hesaplamaları sıfırla
  ...
  }, [diff.yil, brutUcret, rows, selectedYear, is18Or50]);
  ...
  const borclarReportConfig = useMemo((): ReportConfig => {
    ...
    return {
      ...
      infoRows: [
        ...
        { label: "Çıplak Brüt Ücret", value: brutUcret ? `${fmtLocal(Number(String(brutUcret).replace(/\./g, '').replace(',', '.')))}₺` : "-" },
        { label: "18 yaş altı / 50 yaş üstü", value: is18Or50 ? "Evet" : "Hayır" },
      ],
      customSections: [
        {
          ...
          content: (
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '16px', border: '1px solid #999', fontSize: '10px' }}>
              ...
              <tbody>
                <tr>
                  <td style={{ padding: '5px 8px', border: '1px solid #999' }}>Hesaplama</td>
                  <td style={{ textAlign: 'right', padding: '5px 8px', border: '1px solid #999', fontVariantNumeric: 'tabular-nums' }}>
                    {is18Or50 ? `3 hafta × ${diff.yil} yıl` : `2 hafta × ${diff.yil} yıl`}
                  </td>
                </tr>
                <tr style={{ fontWeight: 600, backgroundColor: '#f3f4f6' }}>
                  <td style={{ padding: '5px 8px', border: '1px solid #999' }}>Toplam İzin Hakkı</td>
  ...
  }, [iseGiris, istenCikis, diff, brutUcret, is18Or50, rows, breakdown, usedTotal, remainingDays, brutIzin, sgk, issizlik, gelirVergisi, gelirVergisiDilimleri, damgaVergisi, netIzin, employerPayment]);
  ...
  const wordTableSections = useMemo(() => {
    ...
    if (borclarReportConfig.infoRows && borclarReportConfig.infoRows.length > 0) {
      const n1 = adaptToWordTable({ headers: ["Alan", "Değer"], rows: borclarReportConfig.infoRows.map(r => [r.label, String(r.value ?? "-")]) });
      sections.push({ id: "ust-bilgiler", title: "Genel Bilgiler", html: buildWordTable(n1.headers, n1.rows) });
    }

    const izinHesaplamaRows = [
      ["Hesaplama", is18Or50 ? `3 hafta × ${diff.yil} yıl` : `2 hafta × ${diff.yil} yıl`],
      ["Toplam İzin Hakkı", `${breakdown.total} gün`],
    ];
    const n2 = adaptToWordTable({ headers: ["Alan", "Değer"], rows: izinHesaplamaRows });
    sections.push({ id: "yillik-izin-hesaplama", title: "Yıllık İzin Hesaplama", html: buildWordTable(n2.headers, n2.rows) });
    ...
    if (validRows.length > 0) {
      ...
      const n3 = adaptToWordTable({ headers: ["Başlangıç Tarihi", "Bitiş Tarihi", "Gün Sayısı"], rows: exclRows });
      sections.push({ id: "kullanilan-izinler", title: "Kullanılan İzinler", html: buildWordTable(n3.headers, n3.rows) });
    }
    ...
    const n4 = adaptToWordTable({ headers: ["Alan", "Değer"], rows: calcRows });
    sections.push({ id: "yillik-ucretli-izin-hesaplama", title: "Yıllık Ücretli İzin Hesaplama", html: buildWordTable(n4.headers, n4.rows) });

    const gnd = borclarReportConfig.grossToNetData?.rows;
    if (gnd?.length) {
    ...
    const md = borclarReportConfig.mahsuplasmaData;
    if (md?.rows) {
    ...
    return sections;
  }, [borclarReportConfig, rows, diff, is18Or50, breakdown, usedTotal, remainingDays, brutUcret, brutIzin, employerPayment]);

  const handlePrint = useCallback(() => {
    const targetEl = document.getElementById("borclar-print-wrapper");
    if (!targetEl) {
      window.print();
      return;
    }
    const title = borclarReportConfig.title;
  ...
  const handleSave = () => {
    try {
      ...
      kaydetAc({
        ...
        redirectPath: `/yillik-izin/borclar/:id`,
      });
    } catch (e) {
  ...
  const handleNewCalculation = () => {
    try {
      ...
      if (id) {
        window.location.href = "/yillik-izin/borclar";
      }
    } catch {}
  };
  ...
  return (
    ...
    >
      ...
      <div id="borclar-print-wrapper" style={{ position: "absolute", left: "-9999px", top: 0, visibility: "hidden", width: "16cm", zIndex: -1 }} aria-hidden="true">
        <ReportContentFromConfig config={borclarReportConfig} />
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl dark:shadow-gray-900/50 border border-gray-200 dark:border-gray-700 p-6 sm:p-8">
              {/* Dates - İşe Giriş, İşten Çıkış, Çalışma Süresi eşit genişlikte yan yana */}
              ...
              {/* Çıplak Brüt Ücret - İşe giriş tarihinin altında, tam boy */}
              <div className="mt-6">
                ...
                <input 
                  ...
                  className={`w-full rounded-xl h-11 border px-3 py-2 text-sm font-medium bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-yellow-500 dark:focus:ring-yellow-400 hover:border-gray-400 dark:hover:border-gray-500 transition-all duration-200 ${
                    asgariUcretHatasi ? 'border-red-500 dark:border-red-500 bg-red-50 dark:bg-red-900/20' : 'border-gray-300 dark:border-gray-600'
                  }`}
                />
                {asgariUcretHatasi && (
              ...
              {/* 18 yaş altı / 50 yaş üstü - Mavi kartın hemen üstünde, Çıplak Brüt altında */}
              ...
              {/* Yıllık İzin Hesaplama - ZARİF */}
              <div className="mt-6 p-5 rounded-xl bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 border-l-4 border-blue-500 dark:border-blue-600 shadow-sm hover:shadow-md transition-all duration-200">
                <div className="text-lg font-bold text-gray-900 dark:text-gray-100 text-center flex items-center justify-center gap-3">
                  ...
                  {is18Or50 ? (
                    <span className="flex items-center gap-2">
                      <span className="text-gray-700 dark:text-gray-300">3 hafta</span>
                      <span className="text-blue-500 dark:text-blue-400">×</span>
                      <span className="text-gray-700 dark:text-gray-300">{diff.yil}</span>
                      <span className="text-blue-500 dark:text-blue-400">=</span>
                      <span className="px-3 py-1 rounded-full bg-blue-500 dark:bg-blue-600 text-white font-bold">{breakdown.total} gün</span>
                    </span>
                  ) : (
                    <span className="flex items-center gap-2">
                      <span className="text-gray-700 dark:text-gray-300">2 hafta</span>
                      <span className="text-blue-500 dark:text-blue-400">×</span>
                      <span className="text-gray-700 dark:text-gray-300">{diff.yil}</span>
                      <span className="text-blue-500 dark:text-blue-400">=</span>
                      <span className="px-3 py-1 rounded-full bg-blue-500 dark:bg-blue-600 text-white font-bold">{breakdown.total} gün</span>
                ...
                <div className="w-full flex items-center justify-between px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-300">
                  ...
                  <div className="flex gap-2 items-center">
                    <Button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border-blue-300 dark:border-blue-600 text-blue-700 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all duration-200"
                    >
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
                      </svg>
                      Kaydet
                      <span className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 cursor-help text-xs" title="Girdiğiniz kullanılan izin günlerini bir isim vererek kaydedin. Başka hesaplamalarda tekrar kullanabilirsiniz.">ⓘ</span>
                    </Button>
                    <Button
                      type="button"
                      variant="outline"
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border-green-300 dark:border-green-600 text-green-700 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30 transition-all duration-200"
                    >
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                      </svg>
                      İçe Aktar
                      <span className="text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300 cursor-help text-xs" title="Daha önce kaydettiğiniz kullanılan izin günlerini yükleyin. Aynı davacı için farklı hesaplamalarda zaman kazandırır.">ⓘ</span>
                    </Button>
                    <Button
                      type="button"
                      variant="outline"
                    ...
                    <div className="overflow-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                          {rows.map((r) => (
                            <tr key={r.id}>
                              <td className="py-2 pr-2"><input type="date" value={r.start} onChange={(e)=>setRow(r.id,{start:e.target.value})} className="w-full rounded-xl h-10 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-2 py-1 text-sm font-medium focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400" /></td>
                              <td className="py-2 pr-2"><input type="date" value={r.end} onChange={(e)=>setRow(r.id,{end:e.target.value})} className="w-full rounded-xl h-10 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-2 py-1 text-sm font-medium focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400" /></td>
              ...
              <div className="mt-3 p-6 rounded-xl bg-gradient-to-br from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 border-l-4 border-yellow-500 dark:border-yellow-600 shadow-sm hover:shadow-md transition-all duration-200">
                ...
                <div className="mt-4 p-3 rounded-lg bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800">
                  <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Davalı tarafından iş akdinin sonlanması ile yıllık ücretli izin bedeli adı altında yapılan ödemedir
                  </label>
                  <input
                    value={employerPayment}
                    onChange={(e)=>setEmployerPayment(e.target.value)}
                    placeholder="Örn: 10.000"
                    className="w-full rounded-xl h-11 border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm font-semibold text-right bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-yellow-500 dark:focus:ring-yellow-400"
                  />
                </div>
              </div>
              ...
              <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-600">
                ...
                <div className="mb-4 p-3 rounded-lg bg-blue-100 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800">
                  <div className="font-semibold text-blue-900 dark:text-blue-300 text-sm">📋 İş Kanunu – Yıllık İzin 14. Madde</div>
                </div>
                <ul className="space-y-3">
                  {NOTE_ITEMS.map((note, index) => (
                    <li key={index} className="flex items-start gap-3 text-slate-600 dark:text-slate-300">
                      <span className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400 text-xs font-bold">{index + 1}</span>
                      <span>{note}</span>
                    </li>
                  ))}
                </ul>
      ...
      <FooterActions
        ...
        previewButton={{
          title: `Borçlar Kanunu ${REPORT_TITLE} Rapor`,
          copyTargetId: "borclar-word-copy",
          ...
          renderContent: () => (
            <div style={{ background: "white", padding: 24 }}>
              <style>{`
                ...
                #borclar-word-copy table { border-collapse: collapse; width: 100%; margin-bottom: 12px; border: 1px solid #999; font-size: 9px; }
                #borclar-word-copy td { border: 1px solid #999; padding: 4px 6px; }
              `}</style>
              <div id="borclar-word-copy">
          ...
          onPdf: () => downloadPdfFromDOM(`Borçlar Kanunu ${REPORT_TITLE} Rapor`, "report-content"),
        }}
      />
